...
require_once($shopRootPath. 'languages/zh_cn/admin/supplier_order.php');
$_LANG = $GLOBALS ['_LANG'];
/* ------------------------------------------------------ */
// --列表
/* ------------------------------------------------------ */
if ($_REQUEST ['act'] == 'list') {
	...
	$smarty->assign ( 'action_link', array (
			'text' => "新增线上订单",
			'href' => 'supplier_m_xian_shang_ding_dan.php?act=add'
	) );
	$agency_list = get_order_list ();
	$smarty->assign ( 'order_list', $agency_list ['orders'] );
	...
	$smarty->display ( 'supplier/supplier_xian_shang_ding_dan_list.dwt' );
}
/* ------------------------------------------------------ */
// -- 排序、分页、查询
/* ------------------------------------------------------ */
...
elseif ($_REQUEST ['act'] == 'info'){
	...
	$sql = "SELECT agency_id FROM " . $ecs->table('admin_user') . " WHERE user_id = '$supplier_id'";
	...
	{	
		...
		$smarty->assign('action_link', array('href' => 'supplier_m_xian_shang_ding_dan.php?act=list&uselastfilter=1', 'text' => $_LANG['01_order_list']));
		$smarty->display('supplier/supplier_xian_shang_ding_dan_info.htm');
	}
...
elseif ($_REQUEST ['act'] == 'edit'){
	...
	elseif ('invoice' == $step)
	{
		...
		if(!empty($_REQUEST['step_detail']))
		{
			...
			{ 
				$action_link = array('text'=>'发票列表','href'=>'supplier_m_xian_shang_ding_dan.php?act=invoice_list&uselastfilter=1');
	...
	/* 显示模板 */
	assign_query_info();
	$smarty->display('supplier/supplier_xian_shang_ding_dan_step.dwt');
...
elseif ($_REQUEST ['act'] == 'insert') {
	...
	// 读取配置
	// 	$xianxiabd_fei = $_CFG ["xianxiabd_fei"] > 0 ? $_CFG ["xianxiabd_fei"] : 0.12; // 报单费
	...
	if ($insertBd) {
		...
		$historyZsq = 0; // 线上消费不在计入累计消费和赠送权
		$consum_money_value = 0;
		$addZsq = 0; // 需要赠送的赠送权数量
		$resUser = log_account_change ( $check_user ["user_id"], 0, 0, 0, $order_amt * (- 1), "会员线上消费，订单记录:" . $bdId . "订单金额" . $order_amt . "", 7, 0, 0, 0, 0, 0 );
		$resAffiliate = true;
		
		$resSupp = log_account_change ( $_SESSION ['user_id'], 0, 0, 0, 0, "会员线上消费，订单记录:" . $bdId . "订单金额" . $order_amt . ",订单收入:" . $hksr, 7, 0, 0, 0, 0, 0, 0, 0, $hksr, 0, 0, 0 );
		$rebeat = array (
		...
		if ($resUser && $resSupp && $insertRebeat && $resAffiliate) {
			...
			admin_log ( "提交线上订单", 'add', 'role_offline_man' );
			...
			$links = array (
					array (
							'href' => 'supplier_m_xian_shang_ding_dan.php?act=list',
							'text' => "线上订单列表"
					),
					array (
							'href' => 'supplier_m_xian_shang_ding_dan.php?act=add',
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
	...
	$smarty->assign('action_link', array('href' => 'supplier_m_xian_shang_ding_dan.php?act=delivery_list&uselastfilter=1', 'text' => $_LANG['09_delivery_order']));
	$smarty->assign('action_act', ($delivery_order['status'] == 2) ? 'delivery_ship' : 'delivery_cancel_ship');
	$smarty->display('supplier/delivery_info.htm');
	exit; //
}
/*------------------------------------------------------ */
//-- 发货单发货确认
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
	...
	if(!empty($delivery_stock_result))
	{
		foreach ($delivery_stock_result as $value)
		{
			...
			{
				/* 操作失败 */
				$links[] = array('text' => $_LANG['order_info'], 'href' => 'supplier_m_xian_shang_ding_dan.php?act=delivery_info&delivery_id=' . $delivery_id);
	...
	{
		/* 操作失败 */
		$links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'supplier_m_xian_shang_ding_dan.php?act=delivery_info&delivery_id=' . $delivery_id);
	...
	if ($order_finish)
	{
		...
		pushUserMsg($order['user_id'],array('order_no'=>$order['order_id'],'order_sn'=>$order['order_sn'],'shoping_code'=>$invoice_no,'shoping_type'=>$order['shipping_name']),2);
		//$_SERVER['REQUEST_URI'] = $_SERVER['REQUEST_URI'] ? $_SERVER['REQUEST_URI'] : "/admin/";
		//$autoUrl = str_replace($_SERVER['REQUEST_URI'], '', $GLOBALS['ecs']->url());
		@file_get_contents("http://".$_SERVER['HTTP_HOST'].'/mobile/weixin/auto_do.php?type=2');
	}
	...
	/* 操作成功 */
	$links[] = array('text' => $_LANG['09_delivery_order'], 'href' => 'supplier_m_xian_shang_ding_dan.php?act=delivery_list');
	$links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'supplier_m_xian_shang_ding_dan.php?act=delivery_info&delivery_id=' . $delivery_id);
...
elseif ($_REQUEST['act'] == 'delivery_list')
{
	...
	$smarty->assign('sort_update_time', '<img src="../../../../members/images/sort_desc.gif">');
	
	/* 显示模板 */
	$smarty->display('supplier/supplier_xian_shang_ding_dan_delivery_list.htm');
}
...
elseif ($_REQUEST['act'] == 'operate')
{
	...
	elseif (isset($_POST['to_shipping']))
	{
		...
		if (!empty($invoice_no))
		{
			...
			$sql="SELECT supplier_name FROM ".$GLOBALS['ecs']->table("supplier")." WHERE user_id =".$_SESSION['user_id'];
			$supplier_name= $GLOBALS ['db']->getOne($sql);
			/* 获取当前操作员 */
			$delivery['action_user'] = $supplier_name;
			...
			{
				/* 操作失败 */
				$links[] = array('text' => $_LANG['order_info'], 'href' => 'supplier_m_xian_shang_ding_dan.php?act=info&order_id=' . $order_id);
				sys_msg($_LANG['act_false'], 1, $links);
			}
	...
	elseif (isset($_POST['ship']))
	{
		...
		assign_query_info();
		$smarty->display('supplier/supplier_xian_shang_ding_dan_delivery_info.htm');
		exit;
	}
	/* 未发货 */
	...
	elseif (isset($_POST['assign']))
	{
		...
		/* 操作成功 */
		$links[] = array('href' => 'supplier_m_xian_shang_ding_dan.php?act=list&' . list_link_postfix(), 'text' => $_LANG['01_order_list']);
		sys_msg($_LANG['act_ok'], 0, $links);
	}
	/* 订单删除 */
	elseif (isset($_POST['remove']))
	{
		...
		if (!$batch)
		{
			...
			/* 返回 */
			sys_msg($_LANG['order_removed'], 0, array(array('href'=>'supplier_m_xian_shang_ding_dan.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_list'])));
		}
	}
	/* 发货单删除 */
	elseif (isset($_REQUEST['remove_invoice']))
	{
		...
		/* 返回 */
		sys_msg($_LANG['tips_delivery_del'], 0, array(array('href'=>'supplier_m_xian_shang_ding_dan.php?act=delivery_list' , 'text' => $_LANG['return_list'])));
	}
	/* 退货单删除 */
	elseif (isset($_REQUEST['remove_back']))
	{
		...
		/* 返回 */
		sys_msg($_LANG['tips_back_del'], 0, array(array('href'=>'supplier_m_xian_shang_ding_dan.php?act=back_list' , 'text' => $_LANG['return_list'])));
	}
	/* 批量打印订单 */
	elseif (isset($_POST['print']))
	{
		...
		foreach ($order_sn_list as $order_sn)
		{
			...
			{
				if (!admin_priv('order_list', '', false))
				{
					continue;
				}
			}
	...
	{
		$url = 'supplier_m_xian_shang_ding_dan.php?act=delivery_list&order_sn='.$_REQUEST['order_sn'];
	...
	else
	{
		...
		{
			/* 一个订单 */
			ecs_header("Location: supplier_m_xian_shang_ding_dan.php?act=operate_post&order_id=" . $order_id .
		...
		{
			/* 多个订单 */
			ecs_header("Location: supplier_m_xian_shang_ding_dan.php?act=batch_operate_post&order_id=" . $order_id .
					"&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
			exit;
		}
	}
...
elseif ($_REQUEST['act'] == 'operate_post')
{
	...
	elseif ('split' == $operation)
	{
		...
		/* 记录log */
		order_action($order['order_sn'], $arr['order_status'], $shipping_status, $order['pay_status'], $action_note, $supplier_name);
...
function get_order_list() {
	...
	if ($result === false)
	{
		...
		$where = "WHERE o.supplier_id='".getSupplierIdMF()."' ";
		if ($filter['order_sn'])
		{
...
// 	}
	...
	{
		$where .= " AND suppliers_id = '" . $supplier_id. "' ";
	}
...
// 		if ($supplier_id> 0)
// 		{
// 			$where .= " AND d.suppliers_id = '" . $supplier_id. "' ";
// 		}
