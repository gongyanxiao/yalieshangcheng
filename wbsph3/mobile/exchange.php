...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'view') {
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id)) {
        ...
        if ($goods === false) {
            ...
            $picture = get_goods_gallery($goods_id);
            $smarty->assign('pictures',$picture );                    // 商品相册
...
function ajax_exchange_get_goods($children, $min, $max, $ext, $limit, $sort, $order) {
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            'g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img, eg.is_hot ' .
            'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            "WHERE eg.goods_id = g.goods_id AND $where $ext ".  area_sql_limit_cookie()." ORDER BY $sort $order";
    $sql .= " $limit";
