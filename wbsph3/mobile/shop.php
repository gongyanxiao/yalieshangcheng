...
/*------------------------------------------------------ */
/* 缓存编号 */
$cache_id = sprintf('%X', crc32($_SESSION['user_rank'] . '-' . $_CFG['lang']."-".$_COOKIE['area_id']));

if (!$smarty->is_cached('shop.dwt', $cache_id))
{
...
//     $smarty->assign('new_goods',       get_recommend_goods('new'));     // 最新商品
    ...
    $hot_goods_ji_fen  = get_ji_fen_goods_list();
    $smarty->assign('hot_goods_ji_fen',   $hot_goods_ji_fen    );   
    
    $shang_jia_goods = get_shang_jia_goods();
    $smarty->assign('shang_jia_goods',   $shang_jia_goods    );
    
    
    $smarty->assign('hot_goods',   $hot_goods    );     // 热销商品, 现在的新品专区
...
$smarty->assign('flash_count',count(get_flash_xml()));
    ...
    $sql="select real_name, SUBSTR(mobile_phone FROM 8 FOR 4) as  mobile_phone from xbmall_users where pai_che_state=1 ";
    $pai_che_users = db_getAll($sql);
    $message ="";
    foreach ($pai_che_users as $key=>$value){
        if($value['real_name']){
            $message.="恭喜(手机尾号".$value['mobile_phone']."）".$value['real_name']."先生加入代理，参与领取奔驰GLC！";
        }else{
            $message.="恭喜(手机尾号".$value['mobile_phone']."）加入代理，参与领取奔驰GLC！  ";
        }
    }
...
$smarty->display('shop.dwt', $cache_id);


function  get_shang_jia_goods(){
    $sql="SELECT g.goods_id, g.goods_name, g.market_price,g.click_count,
 g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, IFNULL(mp.user_price, g.shop_price * '1') AS shop_price, 
g.exclusive, g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief,
 g.goods_type FROM  `xbmall_goods` AS g LEFT JOIN  `xbmall_member_price` AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = '0' 
WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_virtual = 0 AND (( 1 AND supplier_id!=0 ) ) ORDER BY goods_id DESC limit 6";
    $all = db_getAll($sql);
    return $all;
}
/**
 ...
 */
function  get_ji_fen_goods_list(){
   $sql= 'SELECT g.goods_id, g.goods_name as name,  g.goods_name_style, eg.exchange_integral, ' .
    'g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img, eg.is_hot ' .
    'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    "WHERE eg.goods_id = g.goods_id   ORDER BY eg.goods_id desc limit 0,6 ";
