...
require(getShopRoot(). 'members/includes/cls_exchange.php');
...
if ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('action_link', array('href'=>'supplier_m_pei_song_qu_yu.php?act=add&shipping='.$shipping_id,
			'text' => "新建配送区域"));
	$smarty->assign('full_page', 1);
	$smarty->display('supplier/shipping_area_list.dwt');
}
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
	...
	$smarty->display('supplier/shipping_area_info.dwt');
}
...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	else
	{
		...
		$lnk[] = array('text' => "返回", 'href'=>'supplier_m_pei_song_qu_yu.php?act=list&shipping='.$_POST['shipping']);
		$lnk[] = array('text' => "继续添加", 'href'=>'supplier_m_pei_song_qu_yu.php?act=add&shipping='.$_POST['shipping']);
		$lnk[]= array('text' => "商家中心", 'href'=>'user.php?act=supplier_center');
...
elseif ($_REQUEST['act'] == 'update')
{
	...
	else
	{
		...
		$plugin        = getShopRoot().'includes/modules/shipping/'. $shipping_code. ".php";
...
elseif ($_REQUEST['act'] == 'multi_remove')
{
	...
	$links[1]= array('text' => "商家中心", 'href'=>'user.php?act=supplier_center');
