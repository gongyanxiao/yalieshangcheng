...
function get_profile($user_id)
{
    ...
    $info['bank'] = $infos['bank'];
    $info['bank_kh'] = $infos['bank_kh'];
    $info['bank_num'] = $infos['bank_num'];
...
//            }
            ...
            /*
             * 是否有推荐奖励，如果开启并且当前会员有推荐人，则进行订单金额的积分奖励 
             */
            if ($orderInfo['user_id'] > 0 && $orderInfo['supplier_id'] * 1 === 0 && $orderInfo['pay_id'] * 1 > 0) {
                if ($GLOBALS["_CFG"]["recommend_set_open"] * 1 === 1) {
                    $parentInfo = get_user_default($orderInfo['user_id']);
                    if ($parentInfo["parent_id"] > 0) {
                        $change_desc = "推荐会员线上购买自营商品获得推荐赠送，" . sprintf($GLOBALS['_LANG']['order_gift_integral'], $orderInfo['order_sn'], 4);
                        $recommend=$orderInfo["total_fee"] * 1 * $GLOBALS["_CFG"]["recommend_set_percent"] / 100;
                        $recommend_points = floor($recommend*100)/100 ;
                        log_account_change($parentInfo['parent_id'], 0, 0, 0, $recommend_points, $change_desc, 6);
                        $affiliate = "insert into " . $GLOBALS['ecs']->table('affiliate_log') . "(order_id,`time`,`user_id`,`user_name`,`money`,`point`,`separate_type`,`change_desc`) VALUES('" . $order_id . "','" . gmtime() . "','" . $parentInfo['parent_id'] . "','".$orderInfo['user_id']."-".$parentInfo['username']."','".($orderInfo["total_fee"] * 1)."','" . $recommend_points . "',0,'" . $change_desc . "')";
                        $GLOBALS['db']->query($affiliate);
                    }
                }
            }
            /**
             ...
             */
            if ($orderInfo['user_id'] > 0) {
                $currentUser = get_user_default($orderInfo['user_id']);
                $historyZsq = $currentUser["history_zsq"];
                $consum_money_value = $currentUser["consum_money_value"] * 1 + $orderInfo["total_fee"] * 1 * 1;
                $addZsq = intval($consum_money_value / 1000) - $historyZsq; //需要赠送的赠送权数量
                log_account_change($orderInfo["user_id"], 0, 0, 0, 0, "会员线上消费，订单号" . $orderInfo['order_sn'] . "累计消费增加:金额" . $orderInfo["total_fee"] . "", 3, $orderInfo["total_fee"]*1, 0, 0, $addZsq, $addZsq);
                ...
                if($addZsq>0){//用户有赠送权的增加
                	...
                	$sql="update  " . $GLOBALS ['ecs']->table ( 'users' ) . " set gou_wu_quan=gou_wu_quan+".$gouWuQuan.", hlph_points=hlph_points-".($gouWuQuan*100)." where user_id=".$orderInfo["user_id"];
...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            //获取需要支付的log_id
            $order['log_id'] = insert_pay_log($order['order_id'], $order['order_amount'], PAY_ORDER);//;get_paylog_id($order['order_id'], $pay_type = PAY_ORDER);
...
//             $pay_obj = new zhizunbao();
            ...
            $order['pay_online'] = $pay_obj->get_code($order, $payment_info['pay_code'], $orderUrl . "user.php?act=order_detail&order_id=" . $order['order_id']);
            
            $order['pay_online'] ="<div style='margin-top:10px'>".$order['pay_online'].'</div>';            
//             $payment = payment_info($order['pay_id']);
...
//                 $redirectUrl = 'http://xiangbai.king51.com/mobile/user.php?act=order_detail&order_id='.$order['order_id'];
//             }
//             else
//             {
//                 $order['pay_online'] = '';
...
//             //生成新的 pay_log_id
//             $sql = 'SELECT * FROM' . $GLOBALS['ecs']->table('pay_log') . " WHERE order_id = '".$order['order_id']."' AND order_type = '".$pay_type = PAY_ORDER."' AND is_paid = 0";
//             $pay_log = $GLOBALS['db']->getRow($sql);
//             unset($pay_log['log_id'],$pay_log['pay_info']);
//             $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('pay_log'), $pay_log, 'INSERT');
//             $order['log_id'] = $GLOBALS['db']->insert_id();
//             $payInfo = $cmbc_sdk->toCreateOrder($selectTradeType, $order['order_amount'], "商城购物单号" . $order['order_sn'], $order['log_id'], $GLOBALS['ecs']->get_domain() . "/testNotify.php", "" ,$redirectUrl);

//             if ($payInfo['code'] * 1 == 1) {
//                 $sql = "UPDATE " . $GLOBALS['ecs']->table('pay_log') . " SET `pay_info`='" . $payInfo['data'] . "' where `log_id`='" . $order['log_id'] . "'";
//                 $GLOBALS['db']->query($sql);
//                 //生成支付代码
//                 $pay_online = $cmbc_sdk->toGetPayInfo($selectTradeType, $payInfo['data'] ,$signPackage);
//                 $order['pay_online'] =  $pay_online ;
//                 if ($redirectUrl)
//                 {
//                     $order['wx_button'] =  $payInfo['data'] ;
//                 }
//             } else {
//                 $order['pay_online'] = '<br>服务器繁忙，请稍后操作';
//             }          
        }
    }
    else
