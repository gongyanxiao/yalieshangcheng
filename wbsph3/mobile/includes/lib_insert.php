...
function get_cainixihuan() {
    if (!empty($_COOKIE['ECS']['history'])) {
        ...
        $sql = "select * from " . $GLOBALS['ecs']->table('goods') . " where cat_id = " . intval($catid) . " and is_best = 1 ";
            ...
            $str .= '<li><div class="item_wrap">
				...
				<div class="dd">
					...
					<div class="btn" style="padding-top:15px;">
						<a class="compare-btn"  style="display:none" data-goods="' . $goods['goods_id'] .
...
function insert_supplier_list() {
    ...
    if (!empty($city_id) && $city_id > 1) {
        $sql = "SELECT ssc.value,ssc.supplier_id,ssc.code FROM " . $GLOBALS['ecs']->table('supplier') . " as s," . $GLOBALS['ecs']->table('supplier_shop_config') . " as ssc WHERE s.`status` = 1 AND ssc.supplier_id = s.supplier_id AND code in('shop_name','shop_logo') AND (code='shop_city' and value='" . $city_id . "') limit 36";
    } else {
