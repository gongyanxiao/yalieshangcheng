...
class zhifutong {
	...
	public function get_code($order, $payType, $pickupUrl, $paySrc = "MP", $liqType = "T0") {
		...
		$receiveUrl = "http://xbsph.xiangbai315.com/testNotify2.php";
		...
		} else if ($payType == 20) { // 支付宝支付
			$issuerId = "wechat"; // issueId 直联时不为空，必须放在表单中提交。
		}else{
		...
		if ($payType != 99) { // 非网银支付
			...
			if($payType==20 || $isXiaoMi){//微信支付或者使用了小米浏览器
			    $smarty->assign ( 'qrPath', $qrPath);
				$html = $smarty->fetch ( 'payment/qrCode.htm' );
				return $html;
			}else  if($payType==22){
				return  $qrPath;// 返回支付链接
			}else {//其它支付方式
				$html = $smarty->fetch ( 'payment/payform.htm' );
				file_put_contents (   "test.txt", $html, FILE_APPEND );
				return $html;
			}
		} else if($payType==99){//手机端快捷支付(支持储蓄卡,信用卡)
			$html = $smarty->fetch ( 'payment/payform.htm' );
			file_put_contents (   "test.txt", $html, FILE_APPEND );
			return $html;
		}
	...
	public function getShopRootPath() {
		...
		$dir = str_replace ( "\\", "/", $dir );
		$pos = strrpos ( $dir, "mobile/includes/modules/openepay" );
		$dirs = substr ( $dir, 0, $pos );
		return $dirs;
	}
	// 保存二维码
	public function saveQRcode($inputCharset, $pickupUrl, $receiveUrl, $version, $language, $merchantId, $payerName, $payerEmail, $payerTelephone, $orderNo, $orderAmount, $orderCurrency, $orderDatetime, $orderExpireDatetime, $productName, $productPrice, $productNum, $productId, $productDesc, $ext1, $ext2, $extTL, $payType, $issuerId, $signMsg) {
		$dir = dirname ( __FILE__ );
		
		$pos = strrpos ( $dir, "modules\openepay" );
		$dirs = substr ( $dir, 0, $pos );
...
// 		var_dump($html);
		...
		if($payType==22){//支付宝支付
			if($isXiaoMi===false){//不是小米浏览器 
				return  $obj ->qrCode;//返回支付宝的支付链接, 不用生成二维码
			} 
		} 
