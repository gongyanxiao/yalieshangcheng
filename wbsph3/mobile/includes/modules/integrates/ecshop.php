...
class ecshop extends integrate {
    ...
    function edit_user($cfg) {
        ...
        if ((!empty($cfg['md5password'])) && $this->field_pass != 'NULL') {
            ...
            $values[] = $this->field_pass . "='" . $this->compile_password(array(
                        'md5password' => $cfg['md5password'], 'ec_salt' => $ec_salt)) . "'";
            // 重置ec_salt、salt
            //$values[] = "salt = 0";
            $values[] = "ec_salt = '" . $ec_salt . "'";
        }

        if (!empty($cfg['surplus_password'])) {
            $ec_salt1 = rand(1, 9999);
            $values[] = 'surplus_password' . "='" . $this->compile_password(array(
                        'md5password' => md5($cfg['surplus_password']), 'ec_salt' => $ec_salt1)) . "'";
            $values[] = "surplus_salt = '" . $ec_salt1 . "'";
        }
        ...
        if ($values) {
            ...
            if (!empty($cfg['md5password'])) {
                $sql = "select uid from " . $GLOBALS['ecs']->table("supplier_admin_user") . " where user_name='" . $cfg['post_username'] . "' ";
                $uidd = $GLOBALS['db']->getOne($sql);
                if (!empty($uidd)) {
                    $sql = "update " . $GLOBALS['ecs']->table("supplier_admin_user") . " set password='" . $this->compile_password(array(
                                'md5password' => $cfg['md5password'], 'ec_salt' => $cfg['ec_salt'])) . "',ec_salt='" . $cfg['ec_salt'] . "' where uid='" . $uidd . "'";
     ...
     * @return boolean
     */
    function check_erjipassword($password) {
        $sql = "SELECT surplus_password,surplus_salt FROM " . $GLOBALS['ecs']->table("users") . " WHERE `user_id`='" . $_SESSION['user_id'] . "'";
        $user = $GLOBALS['db']->getRow($sql);
        if ($user['surplus_password'] != $this->compile_password(array('md5password' => md5($password), 'ec_salt' => $user['surplus_salt']))) {
            return false;
        }
        return true;
    }
