...
class cls_image {
     ...
     *
     * @access      public
     * @param       array       upload       包含上传的图片文件信息的数
     * @param       array       $rooPath     上传图片的根目录
     ...
     */
    function upload_image_to_dir($upload,$rooPath, $dir = '', $img_name = '')
    {
    	/* 没有指定目录默认为根目录images */
    	if (empty($dir))
    	{
    		/* 创建当月目录 */
    		$dir = date('Ym');
    		$dir = $rooPath.'/'. $this->images_dir . '/' . $dir . '/';
    	}
    	else
    	{
    		/* 创建目录 */
    		$dir = $rooPath.'/'. $this->data_dir . '/' . $dir . '/';
    		if ($img_name)
    		{
    	...
    	{
    		return str_replace($rooPath.'/', '', $img_name);
    	}
    	else
    	{
