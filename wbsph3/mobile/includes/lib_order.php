...
function order_fee($order, $goods, $consignee) {
    ...
    foreach ($goods AS $val) {
        ...
        $total['shop_price'] += $val['shop_price'] * $val['goods_number'];
    ...
    $total['shop_price_formated'] = price_format($total['shop_price'], false);
...
function get_pingtai_rebate_from_supplier($order_id) {
    ...
    $sql = "select *, sum(money_paid + surplus) as jisuan_money , order_amount as jisuan_money1 from " . $ecs->table('order_info') . " where order_id=" . $order_id; // sum(money_paid + surplus) 会员直接支付的钱
    
    $info = $db->getRow($sql);
    
    //判断价格
    if ($info['jisuan_money'] == 0 && $info['jisuan_money1']>0)
    {
        $info['jisuan_money'] = $info['jisuan_money1'];
    }
    ...
    if ($info['pay_status'] == PS_PAYED) {
        ...
        //获取商家佣金比例
        $sql = " select value from ".$GLOBALS['ecs']->table('shop_config')." where `code` = 'supplier_fei'  ";
        $rebates = $GLOBALS['db']->getOne($sql);
        $rebate  = $rebates > 0 ? $rebates : 0.1 ;
        $rebate_money = round(($money * $rebate), 2); //返给平台方的价钱
        $result_money = $money - $rebate_money; //入驻商获取的价钱
        
        $texts = "订单支付";
        $add_time = gmtime();
        //佣金订单日志
        $userDefault = get_user_default($supplier_user_id);
        $sql = "INSERT INTO " . $ecs->table('supplier_rebate_log') . " (order_id,order_sn,supplier_id,all_money,rebate_money,result_money,pay_id,pay_name,texts,add_time,supplier_parent_id,real_supplier_id) VALUES ({$order_id},'{$order_sn}',{$supplier_user_id},'{$money}','{$rebate_money}','{$result_money}',{$pay_id},'{$pay_name}','{$texts}',{$add_time},'" . $userDefault['parent_id'] . "','" . $supplier_id . "')";
        $db->query($sql);

        //入驻商绑定的会员帐户日志变动
        $change_desc = "订单:" . $order_sn . "返入驻商会员货款积分";
        //$user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER, 
        //$consum_money = 0, $give_points = 0, $tx_points = 0, $history_zsq = 0, $zsq = 0, $frozen_points = 0, $day_points = 0, $hk_points = 0, $frozen_hk_points = 0, $yl_points = 0, $jljs_points = 0, $hz_points = 0
        log_account_change($supplier_user_id, 0, 0, 0, 0, $change_desc, ACT_ADJUSTING, 0, 0, 0, 0, 0, 0, 0, $result_money, 0, 0, 0, 0);
    }
    return true;
}
...
function cart_goods($type = CART_GENERAL_GOODS) {
    ...
    $sql_where = $_SESSION['user_id'] > 0 ? "c.user_id='" . $_SESSION['user_id'] . "' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT g.shop_price,c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0) {
    ...
    /* 处理退款 */
    if (1 == $refund_type) {
        log_account_change($user_id, 0, 0, 0, $amount, $change_desc);
...
function get_cart_goods($other = '') {
    ...
    /* 循环、统计 */
    $sql_where = $_SESSION['user_id'] > 0 ? "c.user_id='" . $_SESSION['user_id'] . "' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.*,g.shop_price, g.cat_id, g.brand_id, c.extension_code, IF(ga.act_id, ga.supplier_id, g.supplier_id) as supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid  " .
...
function order_query_sql($type = 'finished', $alias = '') {
    ...
    /* 待收货订单 */ elseif ($type == 'await_receipt') {
        return " AND {$alias}order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) .
