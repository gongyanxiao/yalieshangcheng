...
function insert_user_account($surplus, $amount,$type="0") {
	...
	" VALUES ($fee,$real_amount,'$surplus[user_id]', '', '$amount', '" . gmtime() . "', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,'".$type."')";
...
function get_online_payment_list($include_balance = true) {
    ...
    if (!$include_balance && strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
        $sql .= " AND (pay_code = 'weixin' or pay_code = 'alipay' or pay_code= 'unionpay' or pay_code= 'bank') ";
    } else {
        $sql .= " AND (pay_code = 'alipay' or pay_code= 'unionpay' or pay_code= 'bank') ";
    }
...
function get_user_default($user_id) {
    ...
    $info['give_points'] = $row['give_points'];
    ...
    $info['integral_formatted'] = $row['pay_points'];
