 ...
 * @return boolean
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER, $consum_money = 0, $give_points = 0, $tx_points = 0, $history_zsq = 0, $zsq = 0, $frozen_points = 0, $day_points = 0, $hk_points = 0, $frozen_hk_points = 0, $yl_points = 0, $jljs_points = 0, $hz_points = 0, $djtfh_points = 0, $gz_points = 0,$fxjl_points=0,$sg_zsq=0,$sg_history_zsq=0,$sg_day_points=0) {
...
function get_province_city_fornew($provinceid = 0, $cityid = 0) {
    ...
    $provinceid = $provinceid ? $provinceid : '0';
    $cityid = $cityid ? $cityid : '0';
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['ecs']->table('area') .
            " WHERE cat_id in (" . $provinceid . "," . $cityid . ") order by cat_id";
    $ret = $GLOBALS['db']->GetAll($sql);
    $dizhi = '';
    foreach ($ret as $k => $v) {
        $dizhi .= $v['cat_name'];
...
function pushUserMsg($ecuid, $msg = array(), $type = 1) {
    ...
    if ($type == 1 && $weixinconfig['buynotice'] == 1) {
        ...
        foreach ($msg as $k => $v) {
            $text = str_replace($k, $v, htmlspecialchars_decode($weixinconfig['sendmsg']));
        }
    } elseif ($type == 3 && $weixinconfig['txnotice'] == 1) {
        $text = htmlspecialchars_decode($weixinconfig['txmsg']);
        foreach ($msg as $k => $v) {
            $text = str_replace($k, $v, htmlspecialchars_decode($text));
        }
