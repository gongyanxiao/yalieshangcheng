...
function check_pay_password(password) {
    if (password.indexOf(" ") != -1) {
        $(currentForm).find('#pay_password_notice').html("二级密码不能包含空格");
        _CHECK_RESULT.ERROR = true;
    } else if (password.length < 6) {
        $(currentForm).find('#pay_password_notice').html("二级密码不能少于6位");
        _CHECK_RESULT.ERROR = true;
    } else {
        $(currentForm).find('#pay_password_notice').html("");
        _CHECK_RESULT.ERROR = false;
    }
}
function checkCardExist(card) {
    ...
    {
        $(currentForm).find("#card_notice").html("身份证号格式不正确");
        document.getElementById('card_notice').style.color = '#900';
        _CHECK_RESULT.ERROR = true;
    } else
    {
        var cardObj = null;
        ...
        $.post('register.php?act=check_card', {
            ...
            if (result == 'false') {
                $(currentForm).find("#card_notice").html("身份证号已经注册");
                document.getElementById('card_notice').style.color = '#900';
                _CHECK_RESULT.ERROR = true;
            }
        }, 'text');
...
function check_confirm_pay_password(confirm_password) {
    var password = $(currentForm).find('#pay_password').val();

    if (password.indexOf(" ") != -1) {
        $(currentForm).find('#confirm_pay_password_notice').html("二级确认密码不能包含空格");
        _CHECK_RESULT.ERROR = true;
        return false;
    }
    if (confirm_password.length < 6) {
        $(currentForm).find('#confirm_pay_password_notice').html("二级确认密码不能少于6位");
        _CHECK_RESULT.ERROR = true;
        return false;
    }
    if (confirm_password != password) {
        $(currentForm).find('#confirm_pay_password_notice').html("二级确认密码与二级密码不一致");
        _CHECK_RESULT.ERROR = true;
    } else {
        $(currentForm).find('#confirm_pay_password_notice').html("");
        _CHECK_RESULT.ERROR = false;
    }
}
 ...
 */
function register() {
    var frm = document.forms['mobileForm'];
    //var username = Utils.trim(frm.elements['username'].value);
    //var email = frm.elements['email'].value;
    var password = Utils.trim(frm.elements['password'].value);
    var confirm_password = Utils.trim(frm.elements['pay_password'].value);
