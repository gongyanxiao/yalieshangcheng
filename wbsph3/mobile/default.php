 ...
 * ============================================================================
 * $Author: derek $
 * $Id: default.php 17217 2011-01-19 06:29:08Z derek $
*/
...
if (empty($_GET['encode']))
{
    ...
    header("Location: default.php?encode=$string\n");
...
else
{
    ...
    if(isset($_REQUEST['type']) && $_REQUEST['type'] == '1'){
 	...
 	$pager = get_pager('default.php', $pager['search'], $count, $page, $size);
        $display = 'shop_list';
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        ...
        "AND (( 1 AND supplier_id=0 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock ." ) ".$tag_where." )";
    $count = $db->getOne($sql);
	...
	if ($sort=='salenum')
	{
		 $sql = "SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name, g.market_price,g.click_count, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  AND g.is_virtual = 0 $attr_in ".
                "AND (( 1 AND supplier_id=0  " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .  " group by g.goods_id " .
            "ORDER BY $sort $order";
	}
 ...
 $shop_price_sql =  "if(g.promote_start_date < ".gmtime()."  and g.promote_end_date >  ".gmtime()."  and g.promote_price > 0 and g.promote_price < IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]'), promote_price, IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]'))";
    ...
    /* 分页 */
    $url_format = "default.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" . $_REQUEST['brand']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
    if (!empty($intromode))
    {
