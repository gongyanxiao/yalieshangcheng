...
function action_check_username() {
    ...
    /* 20170415添加找回密码是否启用发送短信验证码开始 */
    $dayu_xiugai = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['ecs']->table("shop_config") . " WHERE code='dayu_xiugai' limit 1");
    $dayu_xiugai=$dayu_xiugai*1;
    $smarty->assign('dayu_xiugai', $dayu_xiugai);
 ...
 * 修改密码
 */
function  action_to_update_password(){
...
function action_reset_password() {
    ...
    $result = $GLOBALS['user']->edit_user(array('username' => $user['user_name'], 'password' => $password, 'surplus_password' => $pay_password));
