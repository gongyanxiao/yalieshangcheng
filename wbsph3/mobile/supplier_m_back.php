...
require($shopRootPath. 'languages/' .$_CFG['lang']. '/admin/common.php');
require($shopRootPath. 'languages/' .$_CFG['lang']. '/admin/log_action.php');

 
if (file_exists($shopRootPath. 'languages/' . $_CFG['lang'] . '/admin/' . basename("back.php")))
{//使用的是会员后台的语言文件
	include($shopRootPath. 'languages/' . $_CFG['lang'] . '/admin/' . basename("back.php"));
}
...
if ($_REQUEST['act'] == 'back_list')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('supplier/back/back_list_2.htm');
}
/*------------------------------------------------------ */
//-- 搜索、排序、分页
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'back_query')
{
    ...
    make_json_result($smarty->fetch('supplier/back/back_list_2.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
...
elseif ($_REQUEST['act'] == 'back_info')
{
    ...
    $smarty->assign('action_link', array('href' => 'supplier_m_back.php?act=back_list&' . list_link_postfix(), 'text' => $_LANG['10_back_order']));
    assign_query_info();
 
    $smarty->display('supplier/back/back_info_2.htm');
    exit; //
}
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['refund']))
    {
        ...
        $smarty->display('supplier/back/back_refund.htm');
		exit;
    } 
	 /* 换出商品寄回 */
	...
	$links[] = array('text' => '返回退款/退货及维修详情', 'href' => 'supplier_m_back.php?act=back_info&back_id=' . $back_id);
...
elseif ($_REQUEST['act'] == 'operate_refund')
{
	...
	if ($_REQUEST['refund_type'] == '1')
	{
	    ...
	    log_account_change($order['user_id'], 0,0,0,$refund_money_2, $desc_back );
	}
...
elseif ($_REQUEST['act'] == 'remove_back')
{
        ...
        /* 返回 */		
        sys_msg('恭喜，记录删除成功！', 0, array(array('href'=>'supplier_m_back.php?act=back_list' , 'text' =>'返回退款/退货及维修列表')));
}
...
elseif ($_REQUEST['act'] == 'replay')
{
        ...
        sys_msg('恭喜，回复成功！', 0, array(array('href'=>'supplier_m_back.php?act=back_info&back_id='.$back_id , 'text' =>'返回')));	
