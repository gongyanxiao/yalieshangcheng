  ...
  <div id="tbh5v0">
    <div class="innercontent1">
      <form method="post" action="user.php" id="edit_profile"   enctype="multipart/form-data" >
        <?php if ($this->_var['profile']['status'] == 1): ?>
         <div class="name" ><span><i style="color:red;">*</i>认证状态</span>
          已认证，审核通过
        </div>
        <?php endif; ?>
        <div class="name" ><span><i style="color:red;">*</i>真实姓名</span>
          <input type="text" <?php if ($this->_var['profile']['status'] == 1): ?>disabled="disabled"<?php endif; ?> name="real_name" value="<?php echo $this->_var['profile']['real_name']; ?>" placeholder="真实姓名" class="c-f-text">
        </div>
       
       
        <div class="name" ><span><i style="color:red;">*</i>身份证号</span>
          <input type="text"   <?php if ($this->_var['profile']['status'] == 1): ?>disabled="disabled"<?php endif; ?> name="card" value="<?php echo $this->_var['profile']['card']; ?>" placeholder="身份证号" class="c-f-text">
        </div> 
        
       
        <div ><span><i style="color:red;">*</i>身份证正面</span>
          <input type="hidden" name="face_card_url"  value="<?php echo $this->_var['profile']['face_card']; ?>" />
              <?php if ($this->_var['profile']['status'] != 1): ?><input type="file" name="face_card" />
                      <br />
              <?php endif; ?>
		      <?php if ($this->_var['profile']['face_card'] != ''): ?>
		      <img src="/<?php echo $this->_var['profile']['face_card']; ?>" width="160" height="160" class="face_img"/>
		      <?php endif; ?>
		      <div class="required-notice"><i class="notice-icon"></i><span class="notice-text people-face-card-notice"></span></div>
        </div>
        
        <div ><span><i style="color:red;">*</i>身份证反面</span>
           <input type="hidden" name="back_card_url"  value="<?php echo $this->_var['profile']['back_card']; ?>" />
                      <?php if ($this->_var['profile']['status'] != 1): ?><input type="file" name="back_card" />
                      <br />
                      <?php endif; ?>
                      <?php if ($this->_var['profile']['back_card'] != ''): ?><img src="/<?php echo $this->_var['profile']['back_card']; ?>" width="150" height="150" class="face_img" />
                      <?php endif; ?>
        ...
        <div class="name" style="display:none;"><span>用户名</span>
          <input type="text" name="user_name" value="<?php echo $this->_var['profile']['user_name']; ?>" placeholder="*用户名" class="c-f-text">
        </div>
        <div class="name" style="display:none;"> <span><?php echo $this->_var['lang']['birthday']; ?></span> <?php echo $this->html_select_date(array('field_order'=>'YMD','prefix'=>'birthday','start_year'=>'-60','end_year'=>'+1','display_days'=>'true','month_format'=>'%m','day_value_format'=>'%02d','time'=>$this->_var['profile']['birthday'])); ?></div>
        <div class="name1" style="display:none;"> <span><?php echo $this->_var['lang']['sex']; ?></span>
          <ul>
...
?>
        ...
        <input type="hidden" name="act" value="act_identity"/>
        <?php endif; ?>
      </form>
    </div>
    <!--
