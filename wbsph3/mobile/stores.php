...
$cache_id = sprintf('%X', crc32(date('ymd') . implode(',', $filter)));
...
function get_all_supplier() {
    ...
    if ($filter['id']) {
        $where .= ' and supplier_category=' . $filter['id'];
    }
     $area_id = $_COOKIE['area_id'];
    if (!empty($area_id)) {
        if ($area_id * 1 == 1) {
     ...
     if (($filter['keywords'] && $filter['keywords'] != '请输入关键词') || !empty($area_id)) {

        if ($filter['keywords'] && empty($area_id)) {
            $is_search = 1;
            ...
            if ($filter['keywords']) {
                ...
                $where .= " and supplier_id in(SELECT DISTINCT supplier_id
				FROM " . $GLOBALS['ecs']->table('supplier_shop_config') . " AS ssc
				WHERE code='shop_city' and value='" . $area_id . "')";
            }
        }
    }
    ...
    foreach ($arr as $key => $val) {
        ...
        $arr[$key]['logo'] = trim($arr[$key]['shop_logo'], ',');
        
        $arr[$key]['cat_name'] = $GLOBALS['db']->getOne("SELECT cat_name from " . $GLOBALS['ecs']->table("category") . " where cat_id='" . $arr[$key]['shop_category'] . "'");
        //添加联系电话
