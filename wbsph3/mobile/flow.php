...
if ($_REQUEST ['act'] == 'ceshiZhiFu') {
	...
	$userId = 199;
	$transSeqId = $_REQUEST ['paySn'];
	$transAmt = sprintf ( "%.2f", $_REQUEST ['transAmt'] );
	$liqType = "T0";
	$merPriv = md5 ( $_REQUEST ['orderId'] . $_REQUEST ['paySn'] );
	$gateId = $_REQUEST ['payCode']; // 网关(付款方式)
	$retUrl = "retUrl";
	$bgRetUrl = $GLOBALS ['ecs']->url () . 'testNotify.php';
// 	$key = "I4rpDZrwiI";
	$key = "LMVPhAQzBc";
	
	$signmsg = strtoupper((md5 ( $merId .  $transSeqId . $transAmt ."0.05" . $merPriv . $gateId ."S"."N". $key ))); // 签名字符串 不可空
	
	// 提交到支付成功页面
	$def_url = '<div style="text-align:center"><form name=\"form\" id=\"payForm\"   action="testNotify.php" method="post" OnSubmit="true">';
	$def_url .= "<input type=text name=\"merId\" value='" . $merId . "'>";
	$def_url .= "<input type=text name=\"transSeqId\" value=" . $transSeqId . ">";
	$def_url .= "<input type=text name=\"transAmt\" value=" . $transAmt . ">";
	$def_url .= "<input type=text name=\"transFee\" value=0.05>";
	$def_url .= "<input type=text name=\"merPriv\" value=" . $merPriv . ">";
	$def_url .= "<input type=text name=\"gateId\" value=" . $gateId . ">";
	$def_url .= "<input type=text name=\"retUrl\" value='" . $retUrl . "'>";
	$def_url .= "<input type=text name=\"bgRetUrl\" value='" . $bgRetUrl . "'>";
	$def_url .= "<input type=text name=\"chkValue\" value=" . $signmsg . ">";
	$def_url .= "<input type=text name=\"paySrc\" value=" . $paySrc . ">";
	$def_url .= "<input type=text name=\"liqType\" value=" . $liqType . ">";
	$def_url .= "<input type=text name=\"transStat\" value=S>";
	$def_url .= "<input type=text name=\"failCase\" value=N>";
	
	$def_url .= "<input type='submit' value='支付成功, 处理订单'  name='submit' style='line-height: 12px;font-size:12px;display: inline-block;text-align: center;text-decoration: none;color: #fff;background-color: #E31939;overflow: visible;border: 0 none;outline: 0;padding: 7px;cursor: pointer'  />";
	$def_url .= "</form></div></br>";
	$def_url;
	die ( $def_url );
}
...
if ($_REQUEST['step'] == 'add_to_cart') {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if($_POST['type']=="xxtz_add"){
            ecs_header("Location: /jygj/wap/mall/xxtz_add.html\n");
        }
        else {
            ecs_header("Location: flow.php?step=checkout\n");
        }
...
//     	$sql="select  exchange_integral from  xbmall_exchange_goods where goods_id=".$cart_goods[$i]['goods_id'];
//     	$temp= $db->getOne($sql);
//     	if($temp){
//     		$totalIntegral+=$temp;
//     	}
...
//    $smarty->assign('cod_disabled',    $cod_disabled);
    ...
    $payment_list = available_payment_list(1, $cod_fee ,true);
    ...
    if ((!isset($_CFG['use_integral']) || $_CFG['use_integral'] == '1') && $_SESSION['user_id'] > 0 && $user_info['pay_points'] > 0 && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
        // 能使用积分
        $keyong = flow_available_points(); // 分红积分
    ...
    if ((!isset($_CFG['use_bonus']) || $_CFG['use_bonus'] == '1') && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type !=
            CART_EXCHANGE_GOODS)) {
        // 取得用户可用红包
...
/* 余额额支付密码_添加_START_www.68ecshop.com */ elseif ($_REQUEST['step'] == 'check_surplus_open') {
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['step'] == 'done') {
    ...
    foreach ($cart_goods_new as $ckey => $cval) {
        ...
        $order['defaultbank'] = $_POST['com_bank'] ? trim($_POST['com_bank']) : "";
...
//			}
        ...
        if ($order['user_id'] > 0 && $order['integral'] > 0) {
            ...
            if ($_CFG['sms_user_money_change'] == 1) {
                $sql = "SELECT pay_points,mobile_phone FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '" . $order['user_id'] . "'";
                $users = $GLOBALS['db']->getRow($sql);
                $content = sprintf($_CFG['sms_use_balance_reduce_tpl'], date("Y-m-d H:i:s", gmtime()), $order['integral'], $users['pay_points'], $_CFG['sms_sign']);
                if ($users['mobile_phone']) {
                    require_once (ROOT_PATH . 'sms/sms.php');
                    sendSMS($users['mobile_phone'], $content);
                }
...
//         $split_order['suborder_list'][$ok]['order_amount_formated'] = price_format($order['order_amount']);
        ...
        if ($order['order_amount'] <= 0) {//余额全额支付(积分支付吧?)
            
            $split_order['suborder_list'][$ok]['order_amount_formated'] = price_format($order['integral'], false);
//             $split_order['suborder_list'][$ok]['order_amount_formated'] = price_format($order['surplus'], false);
        } else {
            if($order['extension_code']=='exchange_goods'){//积分兑换
                $split_order['suborder_list'][$ok]['order_amount_formated'] = price_format($order['integral'], false);
            }else{
        ...
        if ($_CFG['dayu_order_placed'] == '1' && $_CFG['dayu_shop_mobile'] != '') {
            ...
            require_once (ROOT_PATH . 'sms/sms.php');
            sendSMS($_CFG['dayu_shop_mobile'], $aa, $moban);
        }
        ...
        pushUserMsg($order['user_id'], array('order_no' => $new_order_id, 'order_sn' => $order['order_sn']), 1);
        //$_SERVER['REQUEST_URI'] = $_SERVER['REQUEST_URI'] ? $_SERVER['REQUEST_URI'] : "/admin/";
        //$autoUrl = str_replace($_SERVER['REQUEST_URI'], '', $GLOBALS['ecs']->url());
        @file_get_contents("http://" . $_SERVER['HTTP_HOST'] . '/mobile/weixin/auto_do.php?type=1');

        if (empty($order['supplier_id'])) {
            $sql = "SELECT user_id FROM " . $GLOBALS['ecs']->table("weixin_config") . " where id=1";
        } else {
            $sql = "SELECT user_id FROM " . $GLOBALS['ecs']->table("supplier") . " where supplier_id='" . $order['supplier_id'] . "'";
        }
        $real_supplier_id = $GLOBALS['db']->getOne($sql);
        pushUserMsg($real_supplier_id, array('order_no' => $new_order_id, 'order_sn' => $order['order_sn']), 5);
        //$_SERVER['REQUEST_URI'] = $_SERVER['REQUEST_URI'] ? $_SERVER['REQUEST_URI'] : "/admin/";
        //$autoUrl = str_replace($_SERVER['REQUEST_URI'], '', $GLOBALS['ecs']->url());
        @file_get_contents("http://" . $_SERVER['HTTP_HOST'] . '/mobile/weixin/auto_do.php?type=5');
        /* 处理虚拟团购商品 */
        /* 如果订单金额为0 处理虚拟卡 */

        if ($order['order_amount'] <= 0) {
            if($order['integral']>0){//积分购物,发放推荐奖励
                order_jiang_li($order['user_id'], $order['integral']);
            }
...
//	                        send_order_bonus($order['order_id']);
    ...
    if ($order['order_amount'] > 0) {
        ...
        if($order['pay_id']==16 || $order['pay_id']==17 || $order['pay_id']==18 ){
        ...
        $pay_online = $pay_obj->get_code($order, $payment['pay_code'], $orderUrl . "user.php?act=order_detail&order_id=" . $order['order_sn'],"qr");
    ...
    if($order['extension_code']=='exchange_goods'){//如果是积分兑换, 那么在显示的时候, 把积分显示出来
        $order['order_amount_formated'] = price_format($order['integral'], false);
    }
 ...
 * 订单付款的推荐奖励
 */
function  order_jiang_li($user_id, $order_amount){
    $user = db_getRow("select * from ".db_table('users')." where user_id={$user_id}");
    if($user==null)return ;
    //一层推荐人奖励
    tui_jian_jiang_li_yi_ceng($user, $order_amount,"一层推荐人奖励");
    //二层推荐人奖励
    tui_jian_jiang_li_er_ceng($user, $order_amount,"二层推荐人奖励");
    //区域代理奖励,发放到推荐奖励可用余额
    tui_jian_jiang_li_ti_xi($user, $order_amount,"体系奖励");
 ...
 */
function   tui_jian_jiang_li_yi_ceng($user, $order_amount,$message){
    if($user['recommend_user']==null){//一层推荐人为空
        return ;
    }
    $jin_e =  $order_amount*0.2; //一层奖励为20%
    tui_jian_jiang_li($user['recommend_user'], $jin_e,$message);
}
 ...
 * @param unknown $dan_shu
 */
function   tui_jian_jiang_li_er_ceng($user, $order_amount,$message){
    if($user['er_ceng_user']==null){//二层推荐人为空
        return ;
    }
    
    $jin_e = $order_amount*0.15; //二层奖励为15%
    tui_jian_jiang_li($user['er_ceng_user'], $jin_e,$message);
}
 ...
 */
function   tui_jian_jiang_li_ti_xi($user, $order_amount){
    if($user['qu_yu_user']==null){//没有区域代理
        return ;
    }
    
    $jin_e =  $order_amount*0.05;//体系拿5%
...
function tui_jian_jiang_li($user_name, $jin_e,$message){
    ...
    //修改账户
    $sql="update xbmall_users  set user_money = user_money+".$jin_e.",
