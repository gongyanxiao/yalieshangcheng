...
switch ($act) {
    ...
    case "addmenu"://添加菜单
        ...
        if ($_POST) {
            ...
            if ($id > 0)
                $menu = $db->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('weixin_menu') . " WHERE `id` = $id");
            if ($menu['type'] == 3) {
    ...
    case "notice":
        if ($_POST) {
            ...
            $txnotice = intval($_POST ['txnotice']);
            $daynotice = intval($_POST ['daynotice']);
            ...
            $txmsg = getstr($_POST['txmsg']);

            $daymsg = getstr($_POST['daymsg']);


            $offline_supp_notice = intval($_POST ['offline_supp_notice']);
            $offline_supp_msg = getstr($_POST['offline_supp_msg']);

            $offline_user_notice = intval($_POST ['offline_user_notice']);
            $offline_user_msg = getstr($_POST['offline_user_msg']);

            $recharge_notice = intval($_POST ['recharge_notice']);
            $recharge_msg = getstr($_POST['recharge_msg']);



            $user_id = 0;
            if (!empty($_POST["user_id"])) {
                $sql = "select user_id FROM " . $GLOBALS['ecs']->table("users") . " where `mobile_phone`='" . $_POST['user_id'] . "'";
                $user_id = $GLOBALS['db']->getOne($sql);
                if (empty($user_id)) {
                    sys_msg('设置自营商家会员信息不存在', 0, $link);
                }
            }
            $suppliernotice = $_POST['suppliernotice'];
            $suppliermsg = $_POST['suppliermsg'];
            ...
            $ret = $db->query(
					...
					`suppliernotice`	='$_POST[suppliernotice]',
					`suppliermsg`	='$_POST[suppliermsg]',
                                        `offline_supp_notice`	='$_POST[offline_supp_notice]',
                                        `offline_supp_msg`	='$_POST[offline_supp_msg]',
                                        `offline_user_notice`	='$_POST[offline_user_notice]',
                                        `offline_user_msg`	='$_POST[offline_user_msg]',
                                        `recharge_notice`	='$_POST[recharge_notice]',
                                        `recharge_msg`	='$_POST[recharge_msg]',
					`user_id`	='$user_id',
            ...
            $smarty->assign('txmsg', $ret ['txmsg']);
            $smarty->assign('sendmsg', $ret ['sendmsg']);
            $smarty->assign('txnotice', $ret ['txnotice']);
            $smarty->assign('buynotice', $ret ['buynotice']);
            $smarty->assign('sendnotice', $ret ['sendnotice']);
            $smarty->assign('daymsg', $ret ['daymsg']);
            $smarty->assign('daynotice', $ret ['daynotice']);


            $smarty->assign('offline_supp_notice', $ret ['offline_supp_notice']);
            $smarty->assign('offline_supp_msg', $ret ['offline_supp_msg']);
            $smarty->assign('offline_user_notice', $ret ['offline_user_notice']);
            $smarty->assign('offline_user_msg', $ret ['offline_user_msg']);
            $smarty->assign('recharge_notice', $ret ['recharge_notice']);
            $smarty->assign('recharge_msg', $ret ['recharge_msg']);


            if (!empty($ret["user_id"])) {
                $sql = "select mobile_phone FROM " . $GLOBALS['ecs']->table("users") . " where `user_id`='" . $ret['user_id'] . "'";
                $user_id = $GLOBALS['db']->getOne($sql);
                $smarty->assign('user_id', $user_id);
            }
            $smarty->assign('suppliernotice', $ret['suppliernotice']);
            $smarty->assign('suppliermsg', $ret['suppliermsg']);
...
function qcode_list() {
    ...
    if ($result === false) {
        ...
        if ($filter['type'] == 1) {
            ...
            if ($key)
                $ex_where .= " or {$tn}.goods_name like '{$key}'";
