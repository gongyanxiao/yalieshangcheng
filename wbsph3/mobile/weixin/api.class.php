<?php
define('IN_ECS', true);
require_once (dirname(__FILE__).'/../includes/init_wx.php');
...
//					'{$wxid}','{$rs['jf_type']}','{$rs['id']}','{$createtime}','{$createymd}','{$num}')");
	...
	function keywordsToKey($keys){
		...
		if($rs['key']){
			...
			return array("key"=>$rs['key'],'diy_type'=>$diy_type);
		}
		return false;
	}
	/**
