...
if ($event['event'] == "subscribe") { //用户关注
    $tiket = $weixin->getRevTicket();
    $msg = $weixinconfig['followmsg'];
    if (!empty($tiket)) {//存在推荐人注册的时候插入
        $sql = "SELECT ecuid,nickname FROM " . $GLOBALS['ecs']->table("weixin_user") . " WHERE ticket='" . $tiket . "' limit 1";
        $pid = $GLOBALS['db']->getRow($sql);
        if (!empty($pid)) {
            $sql = "SELECT ecuid FROM " . $GLOBALS['ecs']->table("weixin_user") . " WHERE fake_id='" . $wxid . "'";
            $isBind = $GLOBALS['db']->getOne($sql);
            if (empty($isBind)) {
                $msg = $msg . ",您的好友[" . $pid["nickname"] . "]邀请，<a href='{$weburl}mobile/user.php?wxid={$wxid}'>点击绑定</a>";
                $sql = "SELECT parent_id FROM" . $GLOBALS['ecs']->table("bind_record") . " where `wxid`='" . $wxid . "'";
                $parentIdRow = $GLOBALS['db']->getRow($sql);
                if (empty($parentIdRow)) {
                    $sql = "INSERT INTO " . $GLOBALS['ecs']->table("bind_record") . "(`wxid`,`parent_id`)  VALUES('" . $wxid . "','" . $pid['ecuid'] . "') ";
                } else {
                    $sql = "update " . $GLOBALS['ecs']->table("bind_record") . "  SET parent_id='" . $pid['ecuid'] . "' WHERE fake_id='" . $wxid . "'";
                }
...
//查询用户输入是否为指定命令
if ($type == "text") {
    //$userKey = $api->keywordsToKey($content);
    $userKey = $api->keywordsToKey($content);
    //error_log($content, 3,'errors.log');

    if (is_array($userKey))
        $event = array('event' => 'CLICK', 'key' => $userKey['key']);
...
if ($event['event'] == "CLICK") {
    ...
    if (count($event) == 5) {
        ...
        $userKey = $api->keywordsToKey($content);
        //error_log($content, 3,'errors.log');
        //if(is_array($userKey)) $event = array('event'=>'CLICK','key'=>$userKey['key']);
    }
...
if ($reMsg) {
    ...
    if ($content) {
        ...
        echo $weixin->text("" . $xhyData)->reply();
        exit;
    }
}