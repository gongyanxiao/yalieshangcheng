...
$aa = $GLOBALS['db']->getRow("SELECT order_id , order_type FROM ".$GLOBALS['ecs']->table('pay_log')." WHERE log_id = '$out_trade_no'");

if ($aa['order_type']==2 )
{
    // 爱心互助
    $order = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('mutual_info') . "
        WHERE order_id = '$order_id' limit 1");
    $order['order_amount'] = $order['mutual_money'];
}
elseif($aa['order_type'] == 3 )
{
    //投保
    $order = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('insure_info') . "
        WHERE order_id = '$order_id' limit 1");
    $order['order_amount'] = $order['insure_money'];
}
else 
{
    //获取订单信息
