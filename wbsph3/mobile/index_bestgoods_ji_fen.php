...
if (! empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax') {
    ...
    foreach ($goodslist as $val) {
        ...
        $res[]['info'] = $GLOBALS['smarty']->fetch('library/index_best_goods_ji_fen.lbi');
    }
}
die($json->encode($res));
...
function get_index_best($limit = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name,g.click_count, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price,  g.exclusive," . "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " . 'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
