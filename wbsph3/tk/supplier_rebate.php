...
function supplier_rebate_info_list()
{
	...
	if ($result === false)
    {
		...
		$where = (isset($_SESSION['supplier_id']) && intval($_SESSION['supplier_id'])>0) ? 'WHERE sr.real_supplier_id='.intval($_SESSION['supplier_id']) : 'WHERE 1';
...
{
	global $db,$ecs;
	$suppid = intval($_SESSION['supplier_user_id']);
