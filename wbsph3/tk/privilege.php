...
WHERE s.user_name='" . $_SESSION['temp_user_name'] . "'";
        ...
        foreach ($suppliers as $suppKey => $suppValue) {
            switch ($suppValue["role"]) {//周:商家后台角色
                case 6:
                	$suppliers[$suppKey]['real_name'] = "(甜空用户)";
                	break;
                case 7:
                 $suppliers[$suppKey]['real_name'] = "(甜空用户,审核中)";
                		break;
                default :
                    $suppliers[$suppKey]['real_name'] = "废弃的角色" . $suppValue['supplier_name'];
                    break;
            }
        }
...
function action_signin() {
    ...
    if ($row) {
        ...
        $_SESSION['member_role'] = $row['role']; // 
        $_SESSION['member_large_area_id'] = $row['large_area_id']; //
        $_SESSION['member_city_id'] = $row['city_id']; // 
        $_SESSION['member_city_level'] = $row['city_level']; // 
