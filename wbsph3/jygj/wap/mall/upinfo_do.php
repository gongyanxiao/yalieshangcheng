...
if($a=='省份'||$b=='地级市'||$c=='市、县级市') {
	$stmt=$link->prepare("UPDATE `xbmall_users` SET `dpmc`=?,`lxdh`=?,`xxdz`=? WHERE `id`=?");  
	$stmt->bind_param('sssi',$dpmc,$lxdh,$xxdz,$a,$b,$c,$r1['id']); 
} else {
	$stmt=$link->prepare("UPDATE `xbmall_users` SET `dpmc`=?,`lxdh`=?,`xxdz`=?,`a`=?,`b`=?,`c`=? WHERE `id`=?");  
	$stmt->bind_param('ssssssi',$dpmc,$lxdh,$xxdz,$a,$b,$c,$r1['id']); 
}  
if(empty($cpid)) {
	$stmt->execute(); 
	$stmt->close();
