...
if($touZi['yi_tou_zi_ci_shu']+$fenShu>$touZi['ke_tou_zi_ci_shu']+3){//
    ...
    alertAndRelocationHistory("您还可投资{$shengYuCiShu}元,可以继续推荐店铺, 获取更多投资资格");
}
else if($touZi['yi_tou_zi_ci_shu']>=$touZi['ke_tou_zi_ci_shu']  ){
    //已经超出投资数限制,  判断还能不能复投3笔
    if($step1TouZiCount>=3){
        alertAndRelocationHistory("您的投资次数已达到上限{$touZi['ke_tou_zi_ci_shu']},并且已有3笔投资运行中,可以继续推荐店铺, 获取更多投资资格");
    }
}
...
//处在第一阶段的投资,并且没有出局
$sql="select * from zt_xxtz where user='{$operateUser}' and cj_state=0  and ft_state!=1    order  by id asc  ";
...
function  jiangLi($xxtz,$user){
    ...
    //修改投资用户的已投资次数
    $sql=" update xbmall_users set yi_tou_zi_ci_shu = yi_tou_zi_ci_shu+1 where user='{$xxtz['user']}' ";
    mysql_query($sql);
    ...
    while($tuiJianRen&& $dongShiNum <3   ){//直到懂事层数达到3层,或没有推荐人为止
        if (empty($tuiJianRen['parent_id']))break;// 或没有推荐人了
...
function  yiCengJiangLi($xxtz,$user,&$zhongXinNum,&$dongShiNum){
    ...
    $sql = "select user, parent_id ,level, dian_pu_shu1, dian_pu_shu2,ye_ji from xbmall_users where user_name='{$user['parent_id']}'";
    ...
    if ($tuiJianRen == null)
        ...
        if ($xxtz['type'] == 2) { // 店铺审核
            // 修改推荐人的可投资次数
            $sql = "update  xbmall_users set ke_tou_zi_ci_shu=ke_tou_zi_ci_shu+50  where user='{$xxtz['user']}'";
            mysql_query($sql);
        ...
        if ($touZiShu == 3) {
            // 增加一层店铺数
            $sql = "update xbmall_users set dian_pu_shu1=dian_pu_shu1+1   where user='{$tuiJianRen['user']}'";
            mysql_query($sql);
            $tuiJianRen['dian_pu_shu1'] = $tuiJianRen['dian_pu_shu1'] + 1;
            $zg_userName = getXM($user['user_name']);
            $sql = "insert into zt_xxtz_sj(user,zg_user,rq,zg_level,user_level) values('{$tuiJianRen['user']}','{$user['user_name']}','{$nowDayStr}',1,{$tuiJianRen['level']})";
            mysql_query($sql); // 插入今天满足3单的用户信息
            
            $sql = "insert into zt_xxtzzs (user,zsrq,comment) values('{$tuiJianRen['user']}','{$nowDateStr}','({$zg_userName})成为你的一层有效商家,有效商家个数{$tuiJianRen['dian_pu_shu1']}')";
            mysql_query($sql);
        }
        $sql = "update xbmall_users set ye_ji=ye_ji+3600    where user='{$tuiJianRen['user']}'";
        mysql_query($sql);
        ...
        // $sql="update xbmall_users set tui_jian_shou_yi=tui_jian_shou_yi+{$guanLiFei} where user='{$tuiJianRen['user']}'"; //发放奖励
        // mysql_query($sql);//发放第一层的奖励
        
        // 增加积分变动记录,推荐收益
        $userXM = getXM($user['user_name']);
        if ($xxtz_sj) { // 投资人是今天成为的有效商家
            faFangUserJiFen($tuiJianRen['user'], $guanLiFei, "您推荐的{$userXM}投资一份,由于其今天成为的有效商家,您作为{$shenFen}获得管理费{$guanLiFei}");
            // $sql ="insert into zt_xxtzzs (type,user,xxtzztxjf,zsrq,comment) values(1,'{$tuiJianRen['user']}',{$guanLiFei},'{$nowDateStr}','您推荐的{$userXM}投资一份,由于其今天成为的有效商家,您作为{$shenFen}获得管理费{$guanLiFei}')";
        } else {
            faFangUserJiFen($tuiJianRen['user'], $guanLiFei, "您推荐的{$userXM}投资一份,您作为{$shenFen}获得管理费{$guanLiFei}");
            // $sql ="insert into zt_xxtzzs (type,user,xxtzztxjf,zsrq,comment) values(1,'{$tuiJianRen['user']}',{$guanLiFei},'{$nowDateStr}','您推荐的{$userXM}投资一份,您作为{$shenFen}获得管理费{$guanLiFei}')";
        }
        // mysql_query($sql);
...
function  erCengJiangLi($xxtz,$yiCengTuiJianRen,&$zhongXinNum,&$dongShiNum){
    global $nowDateStr;
    $sql = "select user, parent_id ,level, dian_pu_shu1, dian_pu_shu2,ye_ji from xbmall_users where user_name='{$yiCengTuiJianRen['parent_id']}'";
    ...
    if ($tuiJianRen == null)
        ...
        if ($xxtz['type'] == 2) {
            ...
            if ($xxtz['fk_user'] != $xxtz['user']) { // 是推荐人给开通店铺
                $sql = "update xbmall_users set dian_pu_shu2=dian_pu_shu2+1  where user='{$yiCengTuiJianRen['user']}'";
                mysql_query($sql);
                ...
                $sql = "update xbmall_users set dian_pu_shu2=dian_pu_shu2+1  where user='{$tuiJianRen['user']}'";
                mysql_query($sql);
                $tuiJianRen['dian_pu_shu2'] = $tuiJianRen['dian_pu_shu2'] + 1;
                $zg_userName = getXM($xxtz['user']);
                $sql = "insert into zt_xxtzzs (user,zsrq,comment) values('{$tuiJianRen['user']}','{$nowDateStr}','({$zg_userName})成为你的二层有效商家,二层有效商家个数{$tuiJianRen['dian_pu_shu2']}')";
                mysql_query($sql);
            }
        }
        ...
        faFangUserJiFen($tuiJianRen['user'], $guanLiFei, "第2层{$userXM}投资一份,您作为{$shenFen}获得管理费{$guanLiFei}");
...
function faFangUserJiFen($user,$benCiJiFen,$message){
    ...
    $sql = "update xbmall_users  set xxtzzfwf=xxtzzfwf+{$xxtzzfwf}, xxtzzcbjz=xxtzzcbjz+{$xxtzzcbjz} ,xxtzzsyjz=xxtzzsyjz+{$xxtzzsyjz} ,
 	 xxtzzxfjz=xxtzzxfjz+{$xxtzzxfjz} , xfjf=xfjf+{$xxtzzxfjz} ,  tui_jian_shou_yi=tui_jian_shou_yi+{$tui_jian_shou_yi}  where user='{$user}'";
    mysql_query($sql);
...
function  duoCengJiangLi($xxtz,$shangCengTuiJianRen,&$zhongXinNum,&$dongShiNum){
    global $nowDateStr;
    $sql = "select user, parent_id ,level, dian_pu_shu1, dian_pu_shu2,ye_ji from xbmall_users where user_name='{$shangCengTuiJianRen['parent_id']}'";
...
//         mysql_query($sql); // 发放第一层的奖励
        ...
        faFangUserJiFen($tuiJianRen['user'], $guanLiFei, "{$userXM}投资一份,您作为{$shenFen}获得管理费{$guanLiFei}");
        // 增加积分变动记录
        // $sql ="insert into zt_xxtzzs (type,user,xxtzztxjf,zsrq,comment) values(1,'{$tuiJianRen['user']}',{$guanLiFei},'{$nowDateStr}','{$userXM}投资一份,您作为{$shenFen}获得管理费{$guanLiFei}')";
        // mysql_query($sql);
        return $tuiJianRen;
}
...
/**
 * 获取姓名
 * @param unknown $user  用户账户
 * @return NULL|unknown  用户姓名
 */
function   getXM($user){
    $sql="select xm from xbmall_users where user_name='{$user}'";
    return getOne($sql);
}
...
/**
 * 获取管理费
 * @param unknown $level 推荐层数
 * @param unknown $yunYingNum //运营中心个数
 */
 ...
 * 获取懂事的管理费
 * @param unknown $level 推荐层数
 * @param unknown $dongShiNum 懂事的层数
 * @return number
 */
...
function logMoney($message,$money){
    ...
    $sql = "update xbmall_users set dian_pu_zi_jin=dian_pu_zi_jin-{$jianDianPuJinE} , xxtzztxjf=xxtzztxjf-{$jianZengSongJinE} ,tui_jian_shou_yi=tui_jian_shou_yi-{$jianTuiJianShouYi} where  user='{$user['user_name']}'";
    mysql_query($sql);
    // 插入积分变动记录， 返回用户的积分
...
function   kaiDian(){
    ...
    global  $nowDateStr,$tjrq;
    $next_ft_time = $tjrq+3600*24*45;
    $operateUser=$_COOKIE['ECS']['username'];
    
    
    $sql = "select  is_dian_pu  from  xbmall_users where user_name='{$tuiJianRen}'";
    $is_dian_pu = getOne($sql);
    //     if($is_dian_pu!=1){
    //         alertAndRelocationHistory("推荐人{$tuiJianRen}还没有开通店铺,不能填写");
    //     }
    $operateUserInfo = getUser($operateUser);
    
    $dianPuUser = $_REQUEST['dianPuUser'];//要开通店铺的人
    if(isset($dianPuUser)){ //给别人开店
        $userInfo = getUser($operateUser);
    }else{//自己开店
        $userInfo = getUser($operateUserInfo['parent_id']);//收益人是推荐人的推荐人
        $dianPuUser = $operateUser;
    }
    
    $tuiJianRen = $userInfo['user'];//店铺的推荐人
    
    log3600("为{$operateUser}开通店铺");
    
    //设置店铺开通了
    $sql="update xbmall_users set is_dian_pu=1  where user='{$dianPuUser}' ";
    mysql_query($sql);
    ...
    if(isset($_REQUEST['dianPuUser'])){
        ...
        alertAndRelocation("开店成功", "xxtz_dp.html");
    }
...
function  shengJi($user){
    ...
    if ($user['dian_pu_shu1']>=$yiCengNum && $user['dian_pu_shu2']>=$erCengNum && $user['ye_ji']>=$yeJi){//满足了升级条件
        $sql="update  xbmall_users set level=level+1 where user='{$user['user_name']}'";
        mysql_query($sql);
