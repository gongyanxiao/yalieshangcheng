...
<table width="100%" border="0" height="32px;" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="98%" align="left" background="images/position2.jpg">您当前的位置 >> 商家积分转换记录  
...
<tr>
    <td height="36" colspan="6" align="left" bgcolor="#DBEBFA">
...
<tr>
  <td width="7%" height="29" align="center" bgcolor="#FFFFFF" class="bk"><span class="btdx">

ID</span></td>
  <td width="14%" align="center" class="bk"><span class="btdx">转换日期</span></td>
  <td width="15%" align="center" class="bk"><span class="btdx">用户名</span></td>
  <td width="18%" align="center" class="bk"><span class="btdx">积分操作记录</span></td>
  <td width="14%" align="center" class="bk"><span class="btdx">省份区域</span></td>
  <td width="32%" align="center" class="bk"><span class="btdx">监测来路URL</span></td>
  </tr>
<?php
$sh=isset($_POST['sh'])?strip_tags($_POST['sh']):'';
$sh1=isset($_POST['sh1'])?strip_tags($_POST['sh1']):'';
error_reporting(0);
...
$phpfile = 'zt_b_exchange_record_list.php';
$page= isset($_GET['page'])?$_GET['page']:1;

if($so<>"" or $sh1<>""){

if($so==""){
$q="select *  from zt_b_exchange_record  where   sf like '%$sf%'  ".$key."  order by id desc";
}else{
$q="select *  from zt_b_exchange_record  where (user like '%$so%')   ".$key."   order by id desc";
}

}else{
$q="select *  from zt_b_exchange_record  where id<>'' ".$key."  order by id desc";
}
$query=mysql_query($q);
...
if($so<>"" or $sh1<>""){
if($so==""){
$query1="select *  from zt_b_exchange_record where    (user like '%$so%')   ".$key."   order by id desc limit $page,1000";
}else{
$query1="select *  from zt_b_exchange_record where (user like '%$so%')   ".$key."  order by id desc limit $page,12";
}
}else{
$query1="select *  from zt_b_exchange_record  where id<>'' ".$key."  order by id desc limit $page,12";
}
...
<tr bgcolor="#cccccc">
 <td height="79" align="center" bgcolor="#FFFFFF" class="bk"><?php echo $row1['id']?></td>
    ...
    <td align="center" bgcolor="#FFFFFF" class="bk"><?php echo $row1['sf']?></td>
