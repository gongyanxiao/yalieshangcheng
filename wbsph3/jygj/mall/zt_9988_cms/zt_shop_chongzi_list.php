...
<table width="100%" border="0" height="32px;" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="98%" align="left" background="images/position2.jpg">您当前的位置 >> 充值记录  
&nbsp;&nbsp;&nbsp;&nbsp;<span style="width:500px;">总计： <font color="#FF0000" size="3px;" face="Arial, Helvetica, sans-serif"><?php
...
$sq2="select sum(je) as ze from zt_cz where zt='1'";
$qs2=mysql_query($sq2);
...
<tr>
    <td height="36" colspan="7" align="left" bgcolor="#DBEBFA">
...
编号</span></td>
  ...
  <td align="center" class="bk"><span class="btdx">商家用户</span></td>
  <td align="center" class="bk"><span class="btdx">充值金额</span></td>
  <td align="center" class="bk"><span class="btdx">订单日期</span></td>
  <td align="center" class="bk"><span class="btdx">支付状态</span></td>
  </tr>
<?php
...
$phpfile = 'zt_shop_chongzi_list.php';
$page= isset($_GET['page'])?$_GET['page']:1;

if($so<>"" or $sh1<>""){

if($so==""){
$q="select *  from zt_cz  where  (zt='$sh1' or zt='3'  or zt='1' )  ".$key."  order by id desc";
}else{
$q="select *  from zt_cz  where (user like '%$so%') ".$key."    order by id desc";
}

}else{
$q="select *  from zt_cz where id <>'' ".$key."   order by id desc";
}
...
if($so<>"" or $sh1<>""){
if($so==""){
$query1="select *  from zt_cz where   (zt='$sh1' or zt='3' or zt='1' ) ".$key."   order by id desc limit $page,1000";
}else{
$query1="select *  from zt_cz where (user like '%$so%' ) ".$key."   order by id desc limit $page,12";
}
}else{
$query1="select *  from zt_cz  where id<>'' ".$key."   order by id desc limit $page,12";
}
...
type="checkbox" id="ch" value="<?php echo $row1['id']?>"/></td>
    ...
    <td align="center" bgcolor="#FFFFFF" style="color: #39F;" class="bk"><?php echo $row1['czmc']?></td>
    <td align="center" bgcolor="#FFFFFF" class="bk"><?php echo $row1['user']?></td>
    <td align="center" bgcolor="#FFFFFF" class="bk"><span style="color:red; font-family:Arial, Helvetica, sans-serif;">¥ <?php echo $row1['je']?></span></td>
    <td align="center" bgcolor="#FFFFFF" class="bk"><?php echo $row1['czrq']?></td>
    <td align="center" bgcolor="#FFFFFF" class="bk"><?php
      if($row1['zt']=="1"){
	 	  echo '<font color="green">已支付</font>';
	  }elseif($row1['zt']=="2"){
			  echo '<font color="red">已扣除</font>';  
		  }else{
