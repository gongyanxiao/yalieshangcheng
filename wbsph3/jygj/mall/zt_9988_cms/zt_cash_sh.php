...
if($yh=="13283727779"){
?>
content:'&nbsp;&nbsp;&nbsp;&nbsp;<select name="sh" id="sh"><option value="4">通过</option><option value="1">不通过</option></select><br>备注:<textarea name="bz" id="bz" cols="45" rows="5"></textarea><br>',
<?php
}else{
?>
content:'&nbsp;&nbsp;&nbsp;&nbsp;<select name="sh" id="sh"><option value="2">已处理</option><option value="3">驳回</option><option value="0">未处理</option></select><br>备注:<textarea name="bz" id="bz" cols="45" rows="5"></textarea><br>',
<?php
}
?>
 ok: function(){
...
<table width="100%" border="0" height="32px;" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="98%" align="left" background="images/position2.jpg">您当前的位置 >>提现记录  
&nbsp;&nbsp;<span style="width:500px;"><a href="zt_cash_nsh.php">财务未处理（<?php
...
mysql_select_db($db_database);
									$sq2="select sum(je) as ze from zt_b_cash_record  where zt='0'";
$qs2=mysql_query($sq2);
$sf2=mysql_fetch_assoc($qs2);
echo number_format($sf2['ze'],2);
									?>)</a> </span>&nbsp;&nbsp;<span style="width:500px;">&nbsp;<a href="zt_cash_pass.php">已驳回（<?php
									$sq2="select sum(je) as ze from zt_b_cash_record  where zt='3'";
$qs2=mysql_query($sq2);
$sf2=mysql_fetch_assoc($qs2);
echo number_format($sf2['ze'],2);
									?>） </a><a href="zt_cash_sh.php">&nbsp;已处理（<?php
									$sq3="select sum(je) as ze from zt_b_cash_record  where zt='2'";
$qs3=mysql_query($sq3);
$sf2=mysql_fetch_assoc($qs3);
echo number_format($sf2['ze'],2);
									?>） </a></span></td>
  </tr>
</table>
...
ID</span></td>
  ...
  <td width="7%" align="center" class="bk"><span class="btdx">管理审核</span></td>
  <td width="5%" align="center" class="bk"><span class="btdx">财务</span></td>
...
$phpfile = 'zt_cash_sh.php';
$page= isset($_GET['page'])?$_GET['page']:1;

if($so<>"" or $sh1<>""){
if($so==""){
$q="select *  from zt_b_cash_record where ck='4' and zt='2'   order by id desc";
}else{
$q="select *  from zt_b_cash_record  where (user like '%$so%')  and ck='4' and zt='2'    order by id desc";
}

}else{
$q="select *  from zt_b_cash_record  and ck='4' and zt='2'  order by id desc";
}
$query=mysql_query($q);
...
if($so<>"" or $sh1<>""){
if($so==""){
$query1="select *  from zt_b_cash_record  where ck='4' and zt='2'  order by id desc limit $page,1000";
}else{
$query1="select *  from zt_b_cash_record where (user like '%$so%' ) and ck='4' and zt='2'  order by id desc limit $page,12";
}
}else{
$query1="select *  from zt_b_cash_record where  ck='4' and zt='2'  order by id desc limit $page,12";
}
...
<tr bgcolor="#cccccc">
 <td height="79" align="center" bgcolor="#FFFFFF" class="bk"><?php echo $row1['id']?></td>
    ...
    <td align="center" bgcolor="#FFFFFF" class="bk" style="color: #39F;"><span  style="color:green;font-size:14px;"><?php if($row1['ck']=="4"){echo "已通过";}
	elseif($row1['ck']=="1"){echo "不通过";}elseif($row1['ck']=="3"){echo "<font color='red'>已驳回</font>";}
	else{echo '<font color="green">处理中</font>';}?></span></td>
    <td align="center" bgcolor="#FFFFFF" class="bk" style="color: #39F;"><span style="color:red;font-size:14px;">
      <?php if($row1['zt']=="0"){echo "未处理";}else{echo '<font color="green">已处理</font>';}?>
    </span></td>
    <td align="center" bgcolor="#FFFFFF" class="bk" style="color: #39F;"><span style="color:red;font-size:14px;"><?php echo $row1['sm']?></span></td>
	...
	$yh=$row1['user'];
	$sql1="select xm,sfzh,zhihang,yhkh,khh from zt_bind_bank where ssyh='$yh' order by bind desc";
	$q1=mysql_query($sql1);
	$ss=mysql_fetch_assoc($q1);
	 echo $ss['xm'].'<br>';echo $ss['sfzh'].'<br>';?></span></td>
    <td align="center" bgcolor="#FFFFFF" class="bk"><?=$row1['khh'];?><?

      $userx=$row1['user'];
$sqlx="select zhihang,khdz from zt_bind_bank where ssyh='".$userx."' order by bind desc";
$qs1x=mysql_query($sqlx);
$sf1x=mysql_fetch_assoc($qs1x);
echo $sf1x['khdz'].$sf1x['zhihang'];
    ?></td>
    <td align="center" bgcolor="#FFFFFF" class="bk"><?=$row1['yhkh'];?></td>
    <td align="center" bgcolor="#FFFFFF" class="bk"><?php echo $row1['url']?$row1['url']:"无";?></td>
    <td align="center" bgcolor="#FFFFFF" class="bk">
				    <?php
if($_COOKIE['zt_user2']=="13283727779"&&$row1['ck']!="3"){
	  ?>
  <button type="submit" data-event="ts" style="background-color: #F60;color:#FFF;width:62px;height:25px;cursor:pointer;" name="<?=$row1['id'];?>">管理</button><br>
  <?php
  }
  ?>
	...
	<?php
	  if($row1['ck']!="3") {
		...
		if($row1['zt']=="0" or $row1['zt']==""){
	  	...
	  	echo '<font color="red">已驳回</font>';
	  }
	 
	  ?> 	
	  <?php
	     if($row1['zt']<>"2" and $row1['ck']=="4" and $_COOKIE['zt_user2']!="13283727779"){
	  ?>
