...
echo number_format($sf2['ze'],2);
									...
									$sq2="select sum(je) as ze from zt_b_cash_record  where ck='3'";
$qs2=mysql_query($sq2);
$sf2=mysql_fetch_assoc($qs2);
echo number_format($sf2['ze'],2);
...
$phpfile = 'zt_cash_pass.php';
$page= isset($_GET['page'])?$_GET['page']:1;

if($so<>"" or $sh1<>""){
if($so==""){
$q="select *  from zt_b_cash_record  where ck='3'   order by id desc";
}else{
$q="select *  from zt_b_cash_record  where (user like '%$so%')  and ck='3'   order by id desc";
}
...
if($so<>"" or $sh1<>""){
if($so==""){
$query1="select *  from zt_b_cash_record  where ck='3'  order by id desc limit $page,1000";
}else{
$query1="select *  from zt_b_cash_record where (user like '%$so%' ) and ck='3' order by id desc limit $page,12";
}
}else{
$query1="select *  from zt_b_cash_record  where ck='3' order by id desc limit $page,12";
}
...
<tr bgcolor="#cccccc">
 <td height="79" align="center" bgcolor="#FFFFFF" class="bk"><?php echo $row1['id']?></td>
    ...
    <td align="center" bgcolor="#FFFFFF" class="bk" style="color: #39F;"><span style="color:red;font-size:14px;">
      <?php if($row1['zt']=="0"){echo "未处理";}elseif($row1['zt']=="3"){echo "已驳回";}else{echo '<font color="green">已处理</font>';}?>
    </span></td>
...
echo $sf1x['khdz'];
    ?>
    	<?=$row1['zh']==''?$sf1x['zhihang']:$row1['zh']?></td>
