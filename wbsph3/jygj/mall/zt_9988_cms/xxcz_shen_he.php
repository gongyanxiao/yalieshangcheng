...
if($type==1){//审核通过, 
	...
	$step=1;
	//进入第一阶段，状态为局中
	$sql=" update zt_xxcz set sh_state={$type},shrq={$shrq},step_start_rq={$shrq},cj_state=0, czrq= {$czrq}, step={$step} where id={$id}";
} else{//审核不通过是没有充值日期的
	$sql=" update zt_xxcz set sh_state={$type},shrq={$shrq},  step={$step} where id={$id}";
}
startTrans();//开启事物
mysql_query($sql);
...
// $sql="select * from  zt_xxcz where id={$id}";
  ...
  * 满足条件的升级成为商家()
  */
 function  shengJiShangJia($chongZhi){
 	
 	$sql="select level from xbmall_users where user_name='{$chongZhi['user']}'";
 	$level = getOne($sql);
 	 ...
 	 */
 	$sql="SELECT SUM(czje) FROM `zt_xxcz` WHERE  (user='{$chongZhi['user']}' or   quan_fan_user='{$chongZhi['user']}' or  zhi_jie_user='{$chongZhi['user']}') and  sh_state=1 " ;
 	$czje = getOne($sql);
 	if($czje>=240000){//消费者的及消费者推荐的人消费额或做单金额之和大约等于24万, 把消费者升级为商家
 		$sql="update xbmall_users set level=level+1 where user='{$chongZhi['user']}'";
 		mysql_query($sql);
 		var_dump("消费者{$chongZhi['user']}升级成为商家");
 			//有人升级商家, 触发至直接推荐人升级代理事件
 		shengJiDaiLi($chongZhi['user']);//触发升级代理
...
//发放直推奖励，直接增加提现积分
 function tuiJianJiangLi($chongZhi){
 	...
 	if ($chongZhi['zhi_jie_user']){
 		...
 		mysql_query($sql);
 		var_dump($sql);
 		$comment = "会员{$chongZhi['user']}消费{$xiaoFeiJinE}直推奖励{$money}";
 	...
 	if ($chongZhi['jian_jie_user']){
 		...
 		mysql_query($sql);
 		var_dump($sql);
 		$comment = "会员{$chongZhi['user']}消费{$xiaoFeiJinE}间接奖励{$money}";
 ...
 function levelJiangLiLog($chongZhi,$userAcc,$level,$money){
 	...
 	if ($level==2){
 		$comment = "会员{$chongZhi['user']}消费{$chongZhi['czje']}代理奖励{$money}";
 	}elseif ($level==3){
 		$comment = "会员{$chongZhi['user']}消费{$chongZhi['czje']}运营中心奖励{$money}";
 	}elseif ($level==4){
 		$comment = "会员{$chongZhi['user']}消费{$chongZhi['czje']}股东奖励{$money}";
 	}
 	$time =time();
 	$sql="insert into zt_xxcz_jl(user,zuo_dan_user,jin_e,shou_yi_user,shou_yi_user_level,shou_yi_jin_e,rq,comment)
 	values('{$chongZhi['user']}','{$chongZhi['quan_fan_user']}',{$chongZhi['czje']},'{$userAcc}',$level,$money,{$time},'{$comment}')";
