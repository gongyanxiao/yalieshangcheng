...
while ( $data  = mysql_fetch_array ( $result ) ) {
    ...
    //修改用户的赠送权
    $sql="update xbmall_users set  zsq =CAST(zsq as signed )+{$zsq},jf=0 where user='{$data['user']}'";
    db_execute($sql);
    ...
    $zsqData = getRow($sql);
    if($zsqData){
        $sql="update  zt_zsq  set zsq=CAST(zsq as signed )+{$zsq} where user='{$data['user']}' ";
        db_execute($sql);
    }else{
...
while ( $data  = mysql_fetch_array ( $result ) ) {
   ...
   $sql="update xbmall_users set jf=0, zsq=CAST(zsq as signed )+{$zsq} where user='{$data['user']}' "; //积分清零
   db_execute($sql);
}
