...
</td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>视频路径</td><td bgcolor='ffffff'><input name="video" type="text" id="video" value="<?=$ecmsfirstpost==1?"":ehtmlspecialchars(stripSlashes($r[video]))?>" size="45">
<a onclick="window.open('ecmseditor/FileMain.php?type=2&classid=<?=$classid?>&infoid=<?=$id?>&filepass=<?=$filepass?>&sinfo=1&doing=1&field=video<?=$ecms_hashur[ehref]?>','','width=700,height=550,scrollbars=yes');" title="选择已上传的FLASH"><img src="../data/images/changeflash.gif" border="0" align="absbottom"></a> 
