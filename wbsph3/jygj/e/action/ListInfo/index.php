...
$amyo=array(1,2,null);
$ajifen=array(1,2,3,4,5,null);
$myo=$_GET['myo'];
$jifen=$_GET['jifen'];
if(!in_array($myo,$amyo))
{
	printerror("ErrorUrl","history.go(-1)",1);
}
if(!in_array($jifen,$ajifen))
{
	printerror("ErrorUrl","history.go(-1)",1);
}
if(empty($classid))
...
if(!empty($myo))
{
	$search='&classid='.$classid.'&myo='.$myo;
}
if(!empty($jifen))
{
	$search='&classid='.$classid.'&jifen='.$jifen;
}
...
if($class_r[$classid][islast])//终极栏目
{
	...
	if($jifen==1) {
		...
		$add.="and buyfen between 1000 and 2000";
	} elseif($jifen==3) {
		$add.="and buyfen between 2000 and 4000";
	} elseif($jifen==4) {
		$add.="and buyfen between 4000 and 6000";
	} elseif($jifen==5) {
		$add.="and buyfen > 6000 ";
	}
	$have_class=0;
