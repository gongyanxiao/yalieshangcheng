...
function user_login(back_history) {
    ...
    var pay_password = logform.find('#pay_password');
    ...
    if (pay_password.val() == '') {
        error.css({'visibility': 'visible'});
        pay_password.parents('.item').addClass('item-error');
        error.find('.msg-error-text').html('请输入二级密码');
        return false;
    }
    ...
    if (back_history) {
        Ajax.call('user.php?act=act_login', 'username=' + username.val() + '&password=' + password.val() + '&pay_password=' + pay_password.val() + '&captcha=' + captcha.val() + '&back_act=' + back_act, return_login_back, 'POST', 'JSON');
    } else {
        Ajax.call('user.php?act=act_login', 'username=' + username.val() + '&password=' + password.val() + '&pay_password=' + pay_password.val() + '&captcha=' + captcha.val() + '&back_act=' + back_act, return_login, 'POST', 'JSON');
    }
    return false;
}
...
function closePopWin(){
   if(isCanClose){
     ...
     if(backUlr!=""){
    	  $('form[name="formLogin"]').find('.msg-error-text').css('visibility', 'visible');
          window.location.href = backUlr;//返回到上一页
     }else{
    	 $('.pop-login,.pop-mask').hide();
         $('form[name="formLogin"]').find('.msg-error-text').css('visibility', 'visible');
         top.location.reload();
...
function popHongBao(){
		layer.open({
                    ...
                    , content: ["http://"+document.domain+"/themes/default/huodong/index.html","no"]
                    , yes: function (layero) {
                        closePopWin();
                    }
                });
