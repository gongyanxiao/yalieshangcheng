...
//    }
    ...
    if (empty($result["message"])) {
        ...
        if ($province == 0 || $city == 0 || $district == 0) {
            ...
            $result["code"] = 0;
            $article_id = $db->getOne("SELECT article_id FROM " . $ecs->table('article') . " WHERE cat_id = '-1' ");
            /* 获得文章的信息 */
            $sql = "SELECT a.* " .
...
//        $other['district'] = isset($_COOKIE['area_register_district']) ? $_COOKIE['area_register_district'] : "";
        ...
        if (empty($other['pay_password'])) {
            show_message('二级密码不能为空');
        }
        // 注册类型：email、mobile
...
//             if ($rows > 0) {
//                 show_message("身份证号码已经存在");
//             }
//         } else {
//             show_message("身份证号码不能为空");
//         }
