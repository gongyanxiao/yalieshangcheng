...
if ($_REQUEST ['act'] == 'ceshi') {
	
	include_once ('includes/lib_payment.php');
	
	injoinfimaly ( 204, 2, "测试" );
}
...
/**
 * 把页面传来的内容解码,完成iframe URL式访问
 */
if ($_REQUEST ['act'] == 'getPayIframe') {
	
	$content = $_REQUEST ['content'];
	$content = str_replace ( '-', '+', $content );
	file_put_contents ( "c:/test.txt", $content . "\r\n\r\n", FILE_APPEND );
...
if ($_REQUEST ['step'] == 'add_to_cart') {
	...
	$payment_list = available_payment_list ( 1, $cod_fee, true, $_SESSION ['extension_code'] == 'virtual_good' ? 1 : 0 );
...
elseif ($_REQUEST ['step'] == 'done') {
	...
	// 生成订单
	// $payment_www_com['www_ecshop68_com_alipay_bank'] = $_POST['com_bank'] ? trim($_POST['com_bank']) : "www_ecshop68_com";
	...
	foreach ( $order_info as $ok => $order ) {
		...
		if ($order ['user_id'] > 0 && $order ['integral'] > 0) {
			...
			if ($_CFG ['sms_user_money_change'] == 1) {
				...
				$users = $GLOBALS ['db']->getRow ( $sql );
				$content = sprintf ( $_CFG ['sms_use_balance_reduce_tpl'], date ( "Y-m-d H:i:s", time () ), $order ['integral'], $users ['pay_points'], $_CFG ['sms_sign'] );
				if ($users ['mobile_phone']) {
					require_once (ROOT_PATH . 'sms/sms.php');
					sendSMS ( $users ['mobile_phone'], $content );
				}
		...
		pushUserMsg ( $order ['user_id'], array (
		...
		if (! empty ( $real_supplier_id )) {
			pushUserMsg ( $real_supplier_id, array (
					'order_no' => $new_order_id,
					'order_sn' => $order ['order_sn'] 
			), 5 );
			// $_SERVER['REQUEST_URI'] = $_SERVER['REQUEST_URI'] ? $_SERVER['REQUEST_URI'] : "/admin/";
	...
	if ($order ['order_amount'] > 0) {
		...
		// }
		// $payInfo = $cmbc_sdk->toCreateOrder($selectTradeType, $order['order_amount'], "网上购物，订单号：" . $order['order_sn'], $order['log_id'], $GLOBALS['ecs']->get_domain() . "/testNotify.php", "");
		// if ($payInfo['code'] * 1 == 1) {
		// $sql = "UPDATE " . $GLOBALS['ecs']->table('pay_log') . " SET `pay_info`='" . $payInfo['data'] . "' where `log_id`='" . $order['log_id'] . "'";
		// // echo $sql;exit;
		// $GLOBALS['db']->query($sql);
		// //生成支付代码
		...
		// } else {
		// show_message('系统繁忙,请稍候再试', '我的订单', 'user.php?act=order_list');
...
// 		$pay_obj = new zhifutong();
		...
		$pay_online = $pay_obj->get_code ( $order, $payment ['pay_code'], $orderUrl . "user.php?act=order_detail&order_id=" . $order ['order_sn'], "PC" );
		
		
		$order ['pay_desc'] = $payment ['pay_desc'];
		
		$pay_online = base64_encode ( $pay_online );
		$pay_online = str_replace ( '+', '-', $pay_online ); // 替换掉加号,防止url传输时被转成空格
	...
	$sql = "SELECT pay_id FROM " . $ecs->table ( 'payment' ) . " WHERE pay_code='alipay'";
	$alipay_pay = $db->getOne ( $sql );
	$smarty->assign ( 'alipay_pay', $alipay_pay );
