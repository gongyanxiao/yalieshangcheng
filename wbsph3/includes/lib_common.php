...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true) {
    ...
    if ($res === NULL) {
        ...
        if ($data === false) {
            $sql = "SELECT c.short_name,c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children " .
...
function get_brands($cat = 0, $app = 'brand') {
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag " .
            ...
            " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " . area_sql_limit_cookie() .
 ...
 * 删除目录下的缓存文件
 */
function  clear_tpl_files_in_dirs($dirs){
	$count = 0;
	foreach ($dirs AS $dir) {
		$folder = @opendir($dir);
...
function clear_tpl_files($is_cache = true, $ext = '') {
	$count = 0;
	$count = clear_tpl_files_of_mp($is_cache,$ext);
	$count+= clear_tpl_files_of_pc($is_cache,$ext);
	return $count;
}
 ...
 * @return unknown
 */
function clear_tpl_files_of_pc($is_cache = true, $ext = '') {
	$dirs = array();
	...
	$str_len = strlen($ext);
	$count = 0;
	$count = clear_tpl_files_in_dirs($dirs);
 ...
 * @return unknown
 */
function clear_tpl_files_of_mp($is_cache = true, $ext = '') {
	$dirs = array();
	 
	$shopRootPath= ROOT_PATH."mobile/";
	...
	if ($is_cache) {
		$cache_dir = $shopRootPath. $tmp_dir . '/caches/';
		$dirs[] = $shopRootPath. $tmp_dir . '/query_caches/';
		$dirs[] = $shopRootPath. $tmp_dir . '/static_caches/';
		for ($i = 0; $i < 16; $i++) {
			$hash_dir = $cache_dir . dechex($i);
			$dirs[] = $hash_dir . '/';
		}
		...
		$dirs[] = $shopRootPath. $tmp_dir . '/compiled/';
		$dirs[] = $shopRootPath. $tmp_dir . '/compiled/admin/';
	}
	$str_len = strlen($ext);
	$count = 0;
	$count = clear_tpl_files_in_dirs($dirs);
...
function log_account_change_zsq($user_id = 0,$change_type =ACT_OTHER,$give_points = 0,$zsq = 0,$day_points = 0,$sg_zsq=0, $sg_day_points=0, $change_desc = ''){
	
	
	if($zsq>0){//如果赠送权大于0,分配300养老积分给用户
		
		$yl_points=$yl_points+300*$zsq;
		//原来有的赠送权
		$sql ="select zsq_flag, sg_zsq,zsq,day_points,sg_day_points from  ".$GLOBALS['ecs']->table('users')." where user_id=".$user_id;
		$row = $GLOBALS['db']->getRow($sql);
		
		$zsq_flag= $row['zsq_flag'];
		if($zsq_flag==0){//原有的分红权是偶数
			$dai_ru_zsq=intval($zsq/2);//手工分红权转入消费分红权的数量
		}else{
			if($zsq%2==1){//增加的分红权是奇数, 跟原有的奇数分红权凑成偶数
				$dai_ru_zsq=intval($zsq/2)+1;
			}else{
				$dai_ru_zsq=intval($zsq/2);
			}
		}
		
		//修改赠送权标志
		$zsq_flag = ($zsq_flag+$zsq)%2;
		
		$sql = "update  ".$GLOBALS['ecs']->table('users')." set zsq_flag=".$zsq_flag." where user_id=".$user_id;
		
		$GLOBALS['db']->query($sql);
		
		if($dai_ru_zsq>0){//二送1操作
			if($row['sg_zsq']<$dai_ru_zsq){//剩余的手工分红权小于要转出的分红权
				$dai_ru_zsq = $row['sg_zsq'];
			}
			
			$sql="update ".$GLOBALS['ecs']->table('users')." set sg_zsq=sg_zsq-".$dai_ru_zsq." ,sg_history_zsq=sg_history_zsq-".$dai_ru_zsq
			.", zsq=zsq+".$dai_ru_zsq." , history_zsq=history_zsq+".$dai_ru_zsq." ,consum_money=consum_money+1000*".$dai_ru_zsq." where user_id=".$user_id;
			$GLOBALS['db']->query($sql);
			/* 插入帐户变动记录 */
			$account_log = array(
					'user_id' => $user_id,
					...
					'change_time' => gmtime(),
					'change_desc' => "增加".$zsq."个分红权后带入".$dai_ru_zsq."个互联分红权",
					'change_type' => $change_type,
					'consum_money' => $dai_ru_zsq*1000,
					'give_points' => $give_points,
					'history_zsq' => $dai_ru_zsq,
					'zsq' => $dai_ru_zsq,
	...
	$account_log = array(
			...
			'change_type' => $change_type,
			'give_points' => $give_points,
			'history_zsq' => $history_zsq,
			'zsq' => $zsq,
			'day_points' => $day_points,
			'yl_points' => 0,
			'hz_points' => 0,
			'sg_zsq'=>$sg_zsq,
			'sg_history_zsq' => $sg_history_zsq,
			'sg_day_points' => $sg_day_points
	);
	$queryAccountLog = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $account_log, 'INSERT');
	if ($queryAccountLog) {
		/* 更新用户信息 */
		...
		" give_points = give_points + ('$give_points')," .
		" history_zsq = history_zsq + ('$history_zsq')," .
		" day_points = day_points + ('$day_points')," .
		" yl_points = yl_points + ('$yl_points')," .

		" hz_points = hz_points + ('$hz_points')," .
			" sg_zsq = sg_zsq + ('$sg_zsq')," .
		" sg_history_zsq = sg_history_zsq + ('$sg_history_zsq')," .
		" sg_day_points = sg_day_points + ('$sg_day_points')," .
		" zsq = zsq + ('$zsq')" .
 ...
 * @param   int     $consum_money    累计消费
 * @param   int     $give_points    赠送积分
 * @param   int     $tx_points    提现积分
 * @param   int     $history_zsq    历史赠送权
 * @param   int     $zsq    赠送权
 * @param type $frozen_points 冻结积分
 * @param type $day_points 日奖励积分
 * @param type $hk_points 货款积分
 * @param type $frozen_hk_points 冻结货款积分
 * @param type $yl_points 养老积分
 * @param type $jljs_points 奖励基数
 * @param type $hz_points 互助基金
 * @param  type   $sg_zsq 手工赠送权
 *  @param  type  $sg_history_zsq 手工累计赠送权
 * @param  type   $sg_day_points 手工累计赠送积分
 * @return boolean
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', 
		$change_type = ACT_OTHER, $consum_money = 0, $give_points = 0, $tx_points = 0, $history_zsq = 0, $zsq = 0, $frozen_points = 0, 
		$day_points = 0, $hk_points = 0, $frozen_hk_points = 0, $yl_points = 0, $jljs_points = 0, $hz_points = 0, $djtfh_points = 0, 
		$gz_points = 0,$fxjl_points=0,$sg_zsq=0,$sg_history_zsq=0,$sg_day_points=0) {
		 
	
	if($zsq>0){//如果赠送权大于0,分配养老积分给用户(每分红权300个积分)
		$yl_points=$yl_points+300*$zsq;
		$sg_zsq = $zsq;//手工赠送权和赠送权一起获得
		$history_zsq = $zsq;
		$sg_history_zsq = $zsq;
	}
    /* 插入帐户变动记录 */
    $account_log = array(
        ...
        'day_points' => $day_points,
        'hk_points' => $hk_points,
        'frozen_hk_points' => $frozen_hk_points,
        'yl_points' => $yl_points,
        'jljs_points' => $jljs_points,
        'hz_points' => $hz_points,
        'djtfh_points' => $djtfh_points,
        'gz_points' => $gz_points,
    	'fxjl_points' => $fxjl_points,
    	'sg_zsq'=>$sg_zsq,
    	'sg_history_zsq' => $sg_history_zsq,
        'sg_day_points' => $sg_day_points
    );
    ...
    if ($queryAccountLog) {
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
                ...
                " pay_points = pay_points + ('$pay_points')," .
                " consum_money = consum_money + ('$consum_money')," .
                " give_points = give_points + ('$give_points')," .
                " give_points_step = give_points_step + ('$give_points')," .
                " tx_points = tx_points + ('$tx_points')," .
                " history_zsq = history_zsq + ('$history_zsq')," .
                " frozen_points = frozen_points + ('$frozen_points')," .
                " day_points = day_points + ('$day_points')," .
                " hk_points = hk_points + ('$hk_points')," .
                " frozen_hk_points = frozen_hk_points + ('$frozen_hk_points')," .
                " yl_points = yl_points + ('$yl_points')," .
                " jljs_points = jljs_points + ('$jljs_points')," .
                " hz_points = hz_points + ('$hz_points')," .
                " djtfh_points = djtfh_points + ('$djtfh_points')," .
                " gz_points = gz_points + ('$gz_points')," .
                " fxjl_points = fxjl_points + ('$fxjl_points')," .
                " sg_zsq = sg_zsq + ('$sg_zsq')," .
                " sg_history_zsq = sg_history_zsq + ('$sg_history_zsq')," .
                " sg_day_points = sg_day_points + ('$sg_day_points')," .
                " zsq = zsq + ('$zsq')" .
 ...
 * @param unknown $change_desc
 * @param unknown $gou_wu_quan
 */
function   gou_wu_quan_log($user_id,$change_desc,$gou_wu_quan){
	/* 插入帐户变动记录 */
	$account_log = array(
			'user_id' => $user_id,
			'change_time' => gmtime(),
			'change_desc' => $change_desc,
			'change_type' => ACT_GOU_WU_QUAN,
			'gou_wu_quan' => $gou_wu_quan
	);
	$queryAccountLog = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $account_log, 'INSERT');
	if ($queryAccountLog) {
		return true;
 ...
 * @param unknown $user_id
 * @param unknown $change_desc
 * @param unknown $fxjl_points
 */
function   fen_xiang_jiang_li_ji_fen_log($user_id,$change_desc,$fxjl_points){
	/* 插入帐户变动记录 */
	$account_log = array(
			'user_id' => $user_id,
			'user_money' => 0,
			...
			'change_time' => gmtime(),
			'change_desc' => $change_desc,
			'change_type' => ACT_FEN_XIANG_JIANG_LI,
			'consum_money' => 0,
			'give_points' => 0,
			'tx_points' => 0,
			'history_zsq' => 0,
	...
	if ($queryAccountLog) {
		/* 更新用户信息 */
		$sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
		" SET   fxjl_points = fxjl_points + $fxjl_points" .
 ...
 *
 */
function get_province_city_fornew($provinceid = 0, $cityid = 0) {
    //周辉
    $provinceid = $provinceid ? $provinceid : '0';
    $cityid = $cityid ? $cityid : '0';
    $sql = 'SELECT region_name as cat_name FROM ' . $GLOBALS['ecs']->table('region') .
            " WHERE region_id in (" . $provinceid . "," . $cityid . ") order by region_id";
    $ret = $GLOBALS['db']->GetAll($sql);
    $dizhi = '';
    foreach ($ret as $k => $v) {
...
= 1");
    ...
    } elseif ($type == 5 && $weixinconfig['suppliernotice'] == 1) {
        $text = htmlspecialchars_decode($weixinconfig['suppliermsg']);
        foreach ($msg as $k => $v) {
            $text = str_replace($k, $v, $text);
            $text = $text;
        }
    } elseif ($type == 6 && $weixinconfig['offline_supp_notice'] == 1) {
        $text = htmlspecialchars_decode($weixinconfig['offline_supp_msg']);
        foreach ($msg as $k => $v) {
            $text = str_replace($k, $v, $text);
            $text = $text;
        }
    } elseif ($type == 7 && $weixinconfig['offline_user_notice'] == 1) {
        $text = htmlspecialchars_decode($weixinconfig['offline_user_msg']);
        foreach ($msg as $k => $v) {
            $text = str_replace($k, $v, $text);
            $text = $text;
        }
    } elseif ($type == 8 && $weixinconfig['recharge_notice'] == 1) {
        $text = htmlspecialchars_decode($weixinconfig['recharge_msg']);
        foreach ($msg as $k => $v) {
            $text = str_replace($k, $v, $text);
            $text = $text;
        }
 ...
 * @return array()
 */
function check_user_is_blacklist($user_id) {
    if ($user_id <= 0) {
        return false;
    }

    $sql = "SELECT limit_desc FROM " . $GLOBALS['ecs']->table('user_blacklist') .
            " WHERE user_id = '$user_id' AND is_remove = 1 limit 1";
