 ...
 * @return number
 */
function  getRequestInt($paramName,$default=-1){
    $r=  empty($_REQUEST[$paramName]) ? $default : intval($_REQUEST[$paramName]);
    if($r==$default){
        $r= empty($_POST[$paramName]) ? $default : intval($_POST[$paramName]);
    }
    return $r;
}

function  getRequestFloat($paramName,$default=-1){
    $r=  empty($_REQUEST[$paramName]) ? $default : floatval($_REQUEST[$paramName]);
    if($r==$default){
        $r= empty($_POST[$paramName]) ? $default : floatval($_POST[$paramName]);
    }
    return $r;
}


function  getRequestDouble($paramName,$default=-1){
    $r=  empty($_REQUEST[$paramName]) ? $default : doubleval($_REQUEST[$paramName]);
    if($r==$default){
        $r= empty($_POST[$paramName]) ? $default : doubleval($_POST[$paramName]);
    }
    return $r;
}
 ...
 * @return number
 */
function  getRequestStr($paramName,$default=''){
    $r=    empty($_REQUEST[$paramName]) ? $default : $_REQUEST[$paramName];
    if($r==$default){
           $r= empty($_POST[$paramName]) ? $default :  $_POST[$paramName] ;
       }
       return $r;
}

/**
 * 获取request中的字符串值,进行trim操作
 * @param unknown $paramName
 * @param string $default
 * @return string
 */
function  getRequestStrTrim($paramName,$default=''){
    $r=  empty($_REQUEST[$paramName]) ? $default : trim($_REQUEST[$paramName]);
   
    if($r==$default){
     $r= empty($_POST[$paramName]) ? $default : trim($_POST[$paramName]);
    }
    return $r;
}
