...
function edit_profile($profile) {
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('user_name', 'bank', 'bank_kh', 'bank_num', 'msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone');
...
//             include_once('includes/modules/openepay/zhifutong.php');
            ...
            $order['pay_online'] = $pay_obj->get_code($order, $payment_info['pay_code'], $orderUrl . "user.php?act=order_detail&order_id=" . $order['order_id'], "PC");
            $order['pay_code'] =$payment_info['pay_code'];//支付方式
...
//            $payInfo = $cmbc_sdk->toCreateOrder($selectTradeType, $order['order_amount'], "商城购物，订单号：" . $order['order_sn'], $order['log_id'], $GLOBALS['ecs']->get_domain() . "/testNotify.php", "");
//
//            if ($payInfo['code'] * 1 == 1) {
