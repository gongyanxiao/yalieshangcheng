...
class cls_mysql {
    ...
    function ErrorMsg($message = '', $sql = '') {
        $logfile = ROOT_PATH."data/log/" . date("Y-m-d") . "_error.log";
...
function db_autoExecute($table, $field_values, $mode = 'INSERT', $where = '', $querymode = ''){
    global  $db;
    return  $db->autoExecute($table, $field_values, $mode , $where , $querymode  );
}

function db_getRow($sql, $limited = false){
    global  $db;
    return  $db->getRow($sql, $limited );
}
...
function   db_getOne($sql, $limited = false){
     global  $db;
    return  $db->getOne($sql,$limited);
}
