...
function get_pingtai_rebate_from_supplier($order_id) {
    ...
    if ($info['pay_status'] == PS_PAYED) {
        ...
        //获取商家佣金比例
        $rebate = $GLOBALS['_CFG']['supplier_fei'] > 0 ? $GLOBALS['_CFG']['supplier_fei'] : 0.1 ;
...
function integral_to_give($order) {
    ...
    /* 判断是否预售 */ else if ($order['extension_code'] == PRE_SALE_CODE) {
        include_once(ROOT_PATH . 'includes/lib_goods.php');
...
function get_region_info_fornew($region_id) {
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['ecs']->table('area') .
            " WHERE cat_id = '$region_id' ";
