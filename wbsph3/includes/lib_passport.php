...
function _register($username, $password, $email_or_mobile, $other = array(), $register_type = 'email') {
    ...
    if (!$result) {
        ...
        /* 推荐处理 */
        $affiliate = ($GLOBALS['_CFG']);
        if (isset($affiliate['recommend_set_open']) && $affiliate['recommend_set_open'] * 1 == 1) {
            // 推荐开关开启
...
//			}
        ...
        $other_key_array = array(
            ...
            'msn', 'qq', 'office_phone', 'home_phone', 'country', 'province', 'city', 'district', 'pay_password'
        );
            ...
            foreach ($other as $key => $val) {
                ...
                if (!in_array($key, $other_key_array)) {
                    ...
                    if ($key == "pay_password") {
                        $ec_salt = rand(1, 9999);
                        $md5password = md5($val);
                        $surplus_password = $GLOBALS['user']->compile_password(array('md5password' => $md5password, 'ec_salt' => $ec_salt));
                        unset($other["pay_password"]);
                        $other['surplus_password'] = $surplus_password;
                        $other['is_surplus_open'] = 1;
                        $other['surplus_salt'] = $ec_salt;
                    } else {
...
function register($username, $password, $email, $other = array()) {
    ...
    if (!$GLOBALS['user']->add_user($username, $password, $email)) {
        ...
        // 定义other合法的变量数组
        $other_key_array = array(
            'msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'pay_password'
        );
        $update_data['reg_time'] = local_strtotime(local_date('Y-m-d H:i:s'));
        $update_data['froms'] = WEB_FROM;
        if ($other) {
