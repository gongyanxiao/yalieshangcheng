...
//CopyRight By UUECS QQ909065309
...
if (!is_spider()) {//判断蜘蛛
    ...
    if (!$_COOKIE['area_id']) {
        $area_id = '1';
        $area_name = '全国';
        setcookie("area_id", $area_id, $ctime, '/');
        setcookie("area_name", $area_name, $ctime, '/');
        $smarty->assign('area_name', $area_name);
    }
    //如果有cookies
    if ($_COOKIE['area_id'] && $_COOKIE['area_id'] != '1' && $pname == 'index') {
        $city_id = (int) $_COOKIE['area_id'];

        $sql = 'SELECT region_name as cat_name FROM ' . $ecs->table("region") . ' WHERE region_id = ' . "$city_id";

        $area_name = $db->getone($sql);
        setcookie("area_name", $area_name, $ctime, '/');
        $GLOBALS['smarty']->assign('area_name', $area_name);
        setcookie("area_id", $city_id, $ctime, '/');
    }
}
...
function get_area_cat_tree_id($cat_id = 0) {//powerby-UEUCS_909065309
    ...
    if ($cat_id * 1 > 0) {
        if ($cat_id * 1 === 1) {
            ...
            $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$cat_id' AND is_show='1' and region_type<3 ";
    ...
    if ($parent_id != 0) {
        ...
        $parent_id2 = $GLOBALS['db']->getOne('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$parent_id' AND is_show='1' ");
        if ($parent_id2 != 0) {
            array_push($area_cat_alls, $parent_id2);
            $parent_id3 = $GLOBALS['db']->getOne('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$parent_id2' AND is_show='1' ");
            if ($parent_id3 != 0) {
    ...
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('region') . " WHERE parent_id = '$parent_id'  AND is_show='1' and region_type<3 ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT region_id as cat_id ' .
...
function get_child_area_tree_id($tree_id = 0, $area_cat_alls) {//powerby-UEUCS_909065309
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('region') . " WHERE parent_id = '$tree_id' and region_type<3";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        $child_sql = 'SELECT region_id as cat_id ' .
                'FROM ' . $GLOBALS['ecs']->table('region') .
                " WHERE parent_id = '$tree_id' and region_type<3 ORDER BY sort_order ASC, region_id ASC";
...
function area_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true) {
    static $res = NULL;

    if ($res === NULL) {


        $sql = "SELECT c.region_id as cat_id, c.region_name as cat_name, c.parent_id, c.is_show, c.sort_order,c.day_points, COUNT(s.region_id) AS has_children " .
...
function get_area_cat_tree($cat_id = 0) {
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT region_id as cat_id,region_name as cat_name ,parent_id,is_show,cat_desc ' .
...
function get_child_area_tree($tree_id = 0) {
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        $child_sql = 'SELECT region_id as cat_id, region_name as cat_name, parent_id, is_show,sort_order,cat_desc ' .
                'FROM ' . $GLOBALS['ecs']->table('area') .
...
//    $area_cat_all_sql = "g.area_cat_id " . db_create_in($area_cat_all); //地区格式化
    $area_id = !empty($_REQUEST['area_id']) ? $_REQUEST['area_id'] : 1;
    $parent_id = get_parent_id($area_id);
    if (empty($parent_id)) {
        $area_cat_all_sql = "g.area_cat_id =" . $area_id; //地区格式化
    } else {
        $area_cat_all_sql = "g.area_cat_id ($area_id,$parent_id)"; //地区格式化
    }
//    $tempArea = db_create_in($area_cat_all);
    $where2 = "";
//    if (!empty($tempArea)) {
//        $children_area = "g.cat_id " . db_create_in($area_cat_all);
//        $temp = get_extension_area_goods($children_area);
//        if (!empty($temp)) {
 ...
 * @return string
 */
function area_sql_limit_cookie() {
    $area_id = !empty($_COOKIE['area_id']) ? $_COOKIE['area_id'] : 1;
   
    $parent_id = get_parent_id($area_id);
    if($area_id==1) return "";//周:如果用户所在地为"全国",那么就可以查询全国的商品
    
    if (empty($parent_id)) {
        $area_cat_all_sql = "g.area_cat_id in ($area_id,1)"; //地区格式化
    } else {
        $area_cat_all_sql = "g.area_cat_id in ($area_id,$parent_id,1)"; //地区格式化
    }
//    $area_cat_all = get_area_cat_tree_id($area_id);
//    $area_cat_all_sql = "g.area_cat_id " . db_create_in($area_cat_all); //地区格式化
//    $tempArea = db_create_in($area_cat_all);
...
//CopyRight By UUECS QQ909065309
//Www.UUECS.Com  ECSHOP&&MOBANQU
function get_extension_area_goods($cats) {
    $extension_goods_array = '';
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['ecs']->table('goods_area_cat') . " AS g WHERE $cats";
...
function com_count($goods_id, $arr = false) {
    if (!$arr) {
...
function ask_count($goods_id, $arr = false) {
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('feedback') .
            " WHERE goods_id = '$goods_id' AND msg_status = 1 AND parent_id = 0");
    return $count;
}
