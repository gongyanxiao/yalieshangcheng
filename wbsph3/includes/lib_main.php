...
function get_user_info($id = 0) {
    ...
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_id, u.email,u.user_rank, u.is_bigfamily, u.user_name, u.user_money, u.pay_points' .
            ' FROM ' . $GLOBALS['ecs']->table('users') . ' AS u ' .
            " WHERE u.user_id = '$id'";
...
function assign_ur_here($cat = 0, $str = '') {
    ...
    if ($filename != 'index') {
        ...
        /* 处理无分类的 */ else {
            ...
            /* 虚拟团购 */ elseif ('virtual_group' == $filename) {
 ...
 * @return  void
 */
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1, $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr = '', $url_format = '', $sch_array = '', $filter = 0) {
...
function set_affiliate() {
    $config = ($GLOBALS['_CFG']);
    if (!empty($_GET['u']) && $config['recommend_set_open'] * 1 == 1) {
