 ...
 * @return  int
 */
function insert_user_account($surplus, $amount,$type="0") {
	 
	$fee=$surplus['fee']?$surplus['fee']:0;
	$real_amount=$surplus['real_amount']?$surplus['real_amount']:$amount;//如果没有设置就赋值为$amount
	if($type==3){//消费分红积分兑换,
		$userDefault = get_user_default($_SESSION['user_id']);
		if($userDefault['is_bigfamily']==0){//非大家庭会员12%手续费
			$fee=$amount*12/100;
			$real_amount=$amount-$fee;
		}
	}
	$admin_user = isset($surplus['admin_user'])?$surplus['admin_user']:'';
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('user_account') .
            ' (fee,real_amount,user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid,`type`)' .
            " VALUES ($fee,$real_amount,'$surplus[user_id]', '$admin_user', '$amount', '" . gmtime() . "', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,'".$type."')";
    $GLOBALS['db']->query($sql);
...
function get_online_payment_list($include_balance = true) {
    ...
    if (!$include_balance) {
        $sql .= " AND pay_code <> 'balance' order by pay_order";
    }
 ...
 * @return  array
 */
function get_account_log($user_id, $num, $start, $process_type = "" ,$type=0) {
    $account_log = array();
    ...
    if ($process_type === "") {
        $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('user_account') .
                ...
                " $types AND process_type " . db_create_in(array(SURPLUS_SAVE, SURPLUS_RETURN)) .
                " ORDER BY add_time DESC";
    } else {
        $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('user_account') .
                " WHERE user_id = '$user_id'" .
                " $types AND process_type ='" . $process_type . "' ORDER BY add_time DESC";
    }
    $res = $GLOBALS['db']->selectLimit($sql, $num, $start);
...
function get_user_default($user_id) {
    ...
    $info['gou_wu_quan'] = $row['gou_wu_quan'];
    $info['hlph_points'] = $row['hlph_points'];
    $info['yl_points'] = $row['yl_points'];
    $info['fxjl_points'] = $row['fxjl_points'];
    $info['hz_points'] = $row['hz_points'];

    $info['hk_points'] = $row['hk_points'];
    $info['hk_xf_points'] = $row['hk_xf_points'];
    $info['gz_points'] = $row['gz_points'];
    
    $info['djtfh_points'] = $row['djtfh_points']; //大家庭分红
