...
function get_cainixihuan() {
    if (!empty($_COOKIE['ECS']['history'])) {
        ...
        $sql = "select * from " . $GLOBALS['ecs']->table('goods') . " where cat_id = " . intval($catid) . " and is_best = 1 AND is_delete = 0";
...
function get_evaluation_sumss($goods_id) {
    $sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table('comment') . " WHERE status=1 and  comment_type =0 and id_value =" . intval($goods_id); //status=1表示通过了的评论才算  comment_type =0表示针对商品的评价 感谢zhangyh的提醒
...
function insert_cart_info() {
    $sql_where = $_SESSION['user_id'] > 0 ? "c.user_id='" . intval($_SESSION['user_id']) . "' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
...
function insert_comments($arr) {
    ...
    $cmt = assign_comment(intval($arr['id']), intval($arr['type']));
...
function insert_apply_supplier($user) {
    ...
    $sql = "select * from " . $GLOBALS['ecs']->table('supplier') . " where user_id = " . intval($user['id']) . ' limit 1';
    ...
    if ($shownum == 1) {
        ...
        $GLOBALS['smarty']->assign('supplier_country', $supplier_country);
        /* 供货商等级 */
        $sql = "select rank_name from " . $GLOBALS['ecs']->table('supplier_rank') . " where rank_id = " . intval($supplier['rank_id']);
        $rank_name = $GLOBALS['db']->getOne($sql);
        $supplier['rank_name'] = $rank_name;
        /* 店铺类型 */
        $sql = "select str_name from " . $GLOBALS['ecs']->table('street_category') . " where str_id = " . intval($supplier['type_id']);
...
function insert_goods_sells($arr) {
    ...
    $sql = 'SELECT SUM(goods_number) AS number ' .
            ...
            " WHERE og.order_id = o.order_id and og.goods_id=" . intval($arr['goods_id']);
    $row = $GLOBALS['db']->GetRow($sql);
    if ($row) {
        $number = intval($row['number']);
    } else {
...
//    }
//    if (isset($_COOKIE["area_id"]) && !empty($_COOKIE["area_id"])) {
//        $city_id = intval($_COOKIE["area_id"]);
//    }
//    if (empty($tag_ret)) {//没有标签
//        if (!empty($city_id) && $city_id > 1) {
//            $sql = "SELECT ssc.value,ssc.supplier_id,ssc.code FROM " . $GLOBALS['ecs']->table('supplier_street') . " as ss," . $GLOBALS['ecs']->table('supplier_shop_config') . " as ssc WHERE ss.`status` = 1 AND ssc.supplier_id = ss.supplier_id AND code in('shop_name','shop_logo') AND (code='shop_city' and value='" . $city_id . "') order by ss.sort_order limit 48";
//        } else {
...
//        $tag_id = intval($tag_ret[$arr['id']]['id']);
//        if (!empty($city_id) && $city_id > 1) {
...
//                    " where stm.tag_id=" . $tag_id . " AND ssc.supplier_id = stm.supplier_id AND ss.supplier_id = stm.supplier_id AND code in('shop_name','shop_logo') AND (code='shop_city' and value='" . $city_id . "') order by ss.sort_order limit 48";
//        } else {
//            $sql = " select ssc.value,ssc.supplier_id,ssc.code from " . $GLOBALS['ecs']->table('supplier_tag_map') . " as stm," .
//                    $GLOBALS['ecs']->table('supplier_shop_config') . " as ssc," .
//                    $GLOBALS['ecs']->table('supplier_street') . " as ss " .
...
function insert_goods_collect($arr) {
    $arr['goods_id']=intval($arr['goods_id']);
    $sql = 'select count(user_id) as ids ' . 'from ' . $GLOBALS['ecs']->table('collect_goods') . "as co " . "where co.goods_id=" . intval($arr['goods_id']);
    $row = $GLOBALS['db']->GetRow($sql);
    if ($row) {
        $ids = intval($row['ids']);
    } else {
...
function insert_get_shop_shipping($arr) {
    ...
    $flow_type = intval($arr['flow_type']);
    ...
    if (count($shipping_list) > 0) {
        ...
        $i = 0;
        $sql_where = $_SESSION['user_id'] > 0 ? "user_id='" . intval($_SESSION['user_id']) . "' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
...
function insert_get_area_name() {
    if (!empty($_COOKIE['area_name'])) {
        return $_COOKIE['area_name'];
    } else {
        return '全国';
    }
}
...
//    $sql="select region_id as cat_id,region_name as `cat_name`,`cat_desc` as pinyin from ".$GLOBALS['ecs']->table('region') ." where is_show=1 and parent_id=1 and region_type<3 order by sort_order asc";
//    $rows=$GLOBALS['db']->getAll($sql);
//    
    $dataRows = array();
//    foreach ($rows as $value)
//    {
//        $sql="select  region_id  as cat_id,region_name as cat_name,`cat_desc` as pinyin from ".$GLOBALS['ecs']->table('region') ." where is_show=1 and parent_id='".$value["cat_id"]."' and region_type<3 order by sort_order desc limit 0,10";
//        $tempData=$GLOBALS['db']->getAll($sql);
//        $tempKey=$value["cat_name"];
//        $dataRows[$tempKey]=$tempData;
//    }
    $sql = "select region_id as region_id,region_name as region_name,`cat_desc` as pinyin from " . $GLOBALS['ecs']->table('region') . " where is_show=1 and is_hot=1 and  parent_id>1 and region_type=2 order by sort_order asc limit 0,30";
    $rows1 = $GLOBALS['db']->getALL($sql);
    $data = array("city_list" => $dataRows, "host_city_list" => $rows1);
...
function insert_user_login() {
    ...
    $GLOBALS['smarty']->assign('userinfo', $_SESSION);
    $GLOBALS['smarty']->assign('userid', $_SESSION["user_id"]);
    $val = $GLOBALS['smarty']->fetch('library/user_login.lbi');
