...
if ($_REQUEST['act'] == 'tipword')
{
	...
	$splitResult = json_str_iconv($_REQUEST['word']);
	$json_www_68ecshop_com   = new JSON;
	$result_www_68ecshop_com = array('error' => 0, 'message' => '', 'content' => '');
	
	if(!$splitResult || strlen($splitResult) < 2 || strlen($splitResult) > 30)
	{
        $result_www_68ecshop_com['error']   = 1;
		die($json_www_68ecshop_com->encode($result_www_68ecshop_com));
	}
	$needle = $replace = array();
	$splitResult = str_replace(array(' ','*', "\'"), array('%', '%', ''), $splitResult);
	$needle[] = $splitResult;
	$replace[] = '<strong style="color:cc0000;">'.$splitResult.'</strong>';
	$logdb = array();
	if(preg_match("/^[a-z0-9A-Z]+$/", $splitResult)) {	
    	$sql_qq = "SELECT * FROM " . $ecs->table('keyword') ." WHERE searchengine='ecshop' AND status='1' AND letter LIKE '%$splitResult%' ORDER BY total_search DESC";
	} else {
    	$sql_qq = "SELECT * FROM " . $ecs->table('keyword') ." WHERE searchengine='ecshop' AND status='1' AND word LIKE '%$splitResult%' ORDER BY total_search DESC";
	}
