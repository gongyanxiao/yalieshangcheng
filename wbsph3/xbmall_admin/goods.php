...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    admin_priv('goods_manage_list');

    $cat_id = empty($_REQUEST['cat_id']) ? 0 : intval($_REQUEST['cat_id']);
    $code   = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    $area_cat_id = empty($_REQUEST['area_cat_id']) ? 1 : intval($_REQUEST['area_cat_id']);//UEUCS_364642382
    ...
    if(intval($_REQUEST['supp'])>0){
    	...
    	$smarty->assign('action_link2',  array('href' => 'goods.php?act=addTaoBao', 'text' => "添加淘宝天猫商品"));
    }
    ...
    $smarty->assign('area_cat_list',     area_cat_list(0, $area_cat_id,true,0,true,2));//UEUCS_364642382
	...
	if($_REQUEST['is_pass']){
    $goods_list = goods_list($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1,'',$_REQUEST['is_pass']);
	}else{
...
elseif ($_REQUEST['act'] == 'addTaoBao'){
	...
	$smarty->display('goods_info_tao_bao.htm');
...
//            $goods['promote_end_date'] = local_date('Y-m-d', $goods['promote_end_date']);
        ...
        $smarty->assign('other_cat_list', $other_cat_list);
         ...
         // 扩展地区//UEUCS_364642382
        $other_area_cat_list = array();
        $sql = "SELECT cat_id FROM " . $ecs->table('goods_area_cat') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $goods['other_area_cat'] = $db->getCol($sql);
...
//-- 插入淘宝商品 更新商品
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'insertTaoBao')
{
	...
	if ($is_insert)
	{
		if ($code == '')
		{
			$sql = "INSERT INTO " . $ecs->table('goods') . " (is_pass,reason,area_cat_id,goods_name, goods_name_style, ghost_count, goods_sn, " .
					...
					"VALUES ('$is_pass','$reason','$area_catgory_id','$_POST[goods_name]', '$goods_name_style', '$ghost_count', '$goods_sn', '$catgory_id', " .
	...
	if (isset($_POST['other_area_cat']))//UEUCS_364642382
	{
		handle_other_area_cat($goods_id, array_unique($_POST['other_area_cat']));
	}
	...
	getTaoBaoInfo( $goods_id);
	
	sys_msg("添加商品成功", 0, $link);
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    $smarty->assign('attribute_count_3',        ($attribute_count + 2));
...
//-- 修改货品属性价格
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_attr_price')
{
    check_authz_json('goods_manage');

    $product_id       = intval($_POST['id']);
    $goods_attr_price       = json_str_iconv(trim($_POST['val']));

    /* 修改 */
    $sql = "UPDATE " . $ecs->table('products') . " SET goods_attr_price = '$goods_attr_price' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        clear_cache_files();
        make_json_result($goods_attr_price);
    }
}
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    $product['goods_attr_price']  = $_POST['goods_attr_price'];
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('products') . " (goods_id, goods_attr, product_sn, product_number,goods_attr_price)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "', '" . $product['goods_attr_price'][$key] . "')";
...
/**
 * 获取商品在淘宝的信息
 * @param unknown $id 淘宝的ID
 */
function  getTaoBaoInfo($goodId){
	global $db,$ecs;
	include_once(ROOT_PATH . '/includes/cls_image.php');
	$image = new cls_image($_CFG['bgcolor']);
	...
	$gid = $goodId;//商品id
	$id = $_POST['taobao_id'];
	$num = intval($_POST['taobao_num']) ? intval($_POST['taobao_num']) : 5;
	$istitle = intval($_POST['taobao_istitle']);
	$cnum = intval($_POST['taobao_cnum']);
	...
	if($content){
		...
		foreach($imgArr as $v){
			$sql="insert into {$ecs->table('goods_gallery')} (goods_id,img_url,thumb_url,img_original) value ($gid,'{$v[goods]}','{$v[thumb]}','{$v[source]}')";
			// 				var_dump($sql);
			$db->query($sql);
		}
...
//			$data = $db->getRow($sql1);
            ...
            $GLOBALS['tongbu_user_discount'] = ($data['discount']/100) > 0 ? ($data['discount']/100) : 1  ;   // 要是查不到数据,默认不打折
