...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $role_prv_attr = explode(",",$priv_str);
    /* 查看是否有权限编辑其他管理员的信息 */
    ...
    $sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " .$ecs->table('admin_action').
                 " WHERE parent_id = 0";
    $res = $db->query($sql_query);//查询出所有一级分类
    while ($rows = $db->FetchRow($res))
    {
        $priv_arr[$rows['action_id']] = $rows;
    }
    ...
    foreach ($priv_arr AS $action_id => $action_group)
    {
        ...
        foreach ($action_group['priv'] AS $key => $val)
        {//记录角色已经有的权限
        	if($priv_str == 'all'){//设置选中
        		$priv_arr[$action_id]['priv'][$key]['cando'] =1 ;
        		continue;
        	} 
        		
        	foreach ($role_prv_attr AS $role_key => $role_val){
        		if($val['action_code']==$role_val){//设置选中
        			$priv_arr[$action_id]['priv'][$key]['cando'] = 1 ;
        			break;
        		}
        		$priv_arr[$action_id]['priv'][$key]['cando'] = 0 ;//设置未选中
        	}
