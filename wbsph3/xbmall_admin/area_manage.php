...
function action_list()
{
    ...
    /* 赋值模板显示 */
    assign('ur_here', $_LANG['05_area_list']);
    assign('full_page', 1);
    
    assign_query_info();
    display('area_list.htm');
}
...
function action_add_area()
{
    ...
    if (db_query($sql, 'SILENT')) {
        ...
        make_json_result(fetch('area_list.htm'));
    } else {
...
function action_drop_area()
{
    ...
    if ($exc->drop($id)) {
        ...
        assign('region_arr', $region_arr);
        assign('region_type', $region['region_type']);
