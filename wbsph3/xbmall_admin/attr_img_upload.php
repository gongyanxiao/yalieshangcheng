...
if (!$goods_id or $goods_attr_id == '-1') {
    echo '<center><br>请添加商品后在进行上传商品的操作！</center>';
    exit;
}
//echo $goods_attr_id;
...
//    $sql_www_ecshop68_com = "update " . $GLOBALS['ecs']->table('goods_gallery') . " set is_attr_image='0' where goods_id='$goods_id' and goods_attr_id='$goods_attr_id' ";
    $sql_www_ecshop68_com = "update " . $GLOBALS['ecs']->table('goods_gallery') . " set is_attr_image='0',goods_attr_id='$goods_attr_id' where goods_id='$goods_id' and  img_id='$img_id_www_ecshop68_com'";
    ...
    $db->query($sql_www_ecshop68_com);
    clear_cache_files();
} elseif ($_REQUEST['act'] == 'set_attrimage_cancel') {
    $img_id_www_ecshop68_com = empty($_REQUEST['img_id']) ? 0 : intval($_REQUEST['img_id']);
//    $sql_www_ecshop68_com = "update " . $GLOBALS['ecs']->table('goods_gallery') . " set is_attr_image='0' where goods_id='$goods_id' and goods_attr_id='$goods_attr_id' ";
    $sql_www_ecshop68_com = "update " . $GLOBALS['ecs']->table('goods_gallery') . " set is_attr_image='0',goods_attr_id=0 where goods_id='$goods_id' and  img_id='$img_id_www_ecshop68_com'";
    $db->query($sql_www_ecshop68_com);
    clear_cache_files();
        ...
        <link rel="stylesheet" type="text/css" href="./js/diyUpload/css/diyUpload.css">
            ...
            <body>
                ...
                <div class="gallery_box">
                    <?php
                    $img_list = array();
                    $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE (goods_attr_id= '$goods_attr_id' or goods_attr_id=0) and goods_id = '$goods_id' order by img_sort,img_id ";
                    ...
                    if ($img_list) {
                        ...
                        $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_attr_id=0 and is_attr_image='0' and goods_id = '$goods_id' order by img_sort,img_id ";
                        $img_list = $db->getAll($sql);
                        if ($img_list) {
                            foreach ($img_list as $img) {
                                echo '<div class="gallery_img_box">';
                               if ($img['is_attr_image']) {
                                echo '<div class="shuxingtupian">属性图片</div>';
                                echo '<img src="../' . $img['thumb_url'] . '"  class="gallery_img">';
                                echo '<br><br><a href="?act=drop_image&goods_id=' . $goods_id . '&goods_attr_id=' . $goods_attr_id . '&img_id=' . $img['img_id'] . '" onclick="javascript: return  (confirm(\'确认删除此图片吗\'))">[删除]</a> <a href="?act=set_attrimage_cancel&goods_id=' . $goods_id . '&goods_attr_id=' . $goods_attr_id . '&img_id=' . $img['img_id'] . '">[属性图片]</a><br>顺序:<input type="text" name="sort" value="' . $img['img_sort'] . '" style="width:20px;" onblur="set_sort(this.value,' . $img['img_id'] . ')">';
                                echo '</div>';
                            } else {
                                echo '<img src="../' . $img['thumb_url'] . '"  class="gallery_img">';
                                echo '<br><br><a href="?act=drop_image&goods_id=' . $goods_id . '&goods_attr_id=' . $goods_attr_id . '&img_id=' . $img['img_id'] . '" onclick="javascript: return  (confirm(\'确认删除此图片吗\'))">[删除]</a> <a href="?act=set_attrimage&goods_id=' . $goods_id . '&goods_attr_id=' . $goods_attr_id . '&img_id=' . $img['img_id'] . '">[属性图片]</a><br>顺序:<input type="text" name="sort" value="' . $img['img_sort'] . '" style="width:20px;" onblur="set_sort(this.value,' . $img['img_id'] . ')">';
...
function handle_gallery_image_attr($goods_id, $goods_attr_id, $image_files) {
    ...
    if ($flag) {
        ...
        {
           $image->add_watermark('../'.$img_url,'',$GLOBALS['_CFG']['watermark'], $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']);
        }
        // ------ 添加水英   结束
