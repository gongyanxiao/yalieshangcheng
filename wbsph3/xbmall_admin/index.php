...
//今日注册会员
    ...
    //线上商家个数
    $shop_total_up = $db->getOne('SELECT COUNT(*) FROM ' .$ecs->table('supplier'). ' WHERE status=1 and role = 3 ');
    $smarty->assign('shop_total_up',$shop_total_up);
    
    //联盟商家个数
    $shop_total_down = $db->getOne('SELECT COUNT(*) FROM ' .$ecs->table('supplier'). ' WHERE status=1  and role = 4 ');
    $smarty->assign('shop_total_down',$shop_total_down);
    
    //待审核商家
    $shop_total_up_shen = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('supplier')." WHERE role = 3 and  applynum = 3 AND status in ('0', '-1')");
    $shop_total_down_shen = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('supplier')." WHERE role = 4 and  applynum = 3 AND status in ('0', '-1')");
    $smarty->assign('shop_total_up_shen',$shop_total_up_shen);
    $smarty->assign('shop_total_down_shen',$shop_total_down_shen);
