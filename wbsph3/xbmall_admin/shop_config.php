...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'mail_settings') {
    /* 检查权限 */
    admin_priv('shop_config_mail_view');
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'post') {
    ...
    foreach ($_POST['value'] AS $key => $val) {
        if ($key * 1 == 1069) {
            $hour = trim($val);
            $sql = "SELECT * FROM " . $GLOBALS['ecs']->table("crons") . " WHERE cron_id=3";
            $cronData = $GLOBALS['db']->getRow($sql);
            if (!empty($cronData)) {
                $cron_hour = $hour;
                $cron = array('day' => $cronData['day'], 'week' => $cronData['week'], 'm' => $cronData['minute'], 'hour' => $cron_hour);
                $next = get_next_time($cron);
                /* 20170415添加时间判断问题开始 */
                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table("send_log") . " WHERE send_date='" . local_strtotime(local_date("Y-m-d")) . "'";
                $sendCount = $GLOBALS['db']->getOne($sql);
                if (empty($sendCount)) {
                    $sendCount = 0;
                }
                if ($sendCount > 0) {//如果今天已经赠送，时间推到明天
                    $next = $next + 86400;
                }
                /* 20170415添加时间判断问题开始 */
                $sql = "UPDATE " . $ecs->table('crons') .
                        "SET nextime='$next',  hour = '$cron_hour' " .
                        "WHERE cron_id = 3 LIMIT 1";
                $db->query($sql);
            }
        }
    }
