...
elseif ($_REQUEST['act'] == 'statics_list')
{
	
	
/* 检查权限 */
	admin_priv('13_order_statics');
	...
	/* 显示模板 */
	assign_query_info();
	$smarty->display('statics_order_list.htm');
}

/*增值税发票_添加_START_www.68ecshop.com*/
elseif($_REQUEST['act'] == 'invoice_list')
{
    admin_priv('invoice_manage_list');
...
elseif ($_REQUEST['act'] == 'queryData')
{
	...
	$sort_flag  = sort_flag($order_list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('statics_order_list_data.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
/*------------------------------------------------------ */
//-- 订单佣金计算
/*------------------------------------------------------ */
...
else if ($_REQUEST['act'] == 'export_order_excel')
{
    export_order_excel();
}
...
//导出订单
function export_order_excel()
{
    
   
    $_REQUEST['page_size'] = 100000;
    $order_list = order_list();
   
   
//     $rows = db_getAll("select * from xbmall_order_info order  by add_time desc ");
    ...
    {     
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C1', '收货人');
        $objPHPExcel->getActiveSheet()->setCellValue('D1', '总金额');
        $objPHPExcel->getActiveSheet()->setCellValue('E1', '应付金额');
        $objPHPExcel->getActiveSheet()->setCellValue('F1', '使用积分');
...
//         var_dump($rows);
        ...
        foreach($order_list['orders'] as $key => $val)
        {
            ...
            $objPHPExcel->getActiveSheet()->setCellValueExplicit('A' . $row_num,
                $val['order_sn'],
                PHPExcel_Cell_DataType::TYPE_STRING);
      
           
            $objPHPExcel->getActiveSheet()->setCellValue('B' . $row_num, local_date('m-d H:i', $val['add_time']));
            $objPHPExcel->getActiveSheet()->setCellValue('C' . $row_num, $val['buyer'].$val['consignee'].$val['address']);
            $objPHPExcel->getActiveSheet()->setCellValue('D' . $row_num, $val['formated_total_fee']);
            $objPHPExcel->getActiveSheet()->setCellValue('E' . $row_num, $val['formated_order_amount']);
            $objPHPExcel->getActiveSheet()->setCellValue('F' . $row_num, $val['integral']);
            ...
            foreach ( $all as $key => $value ) {
                ...
                $objPHPExcel->getActiveSheet()->mergeCells('B'.$row_num.':F'.$row_num);
            }
            if(count($all)>0){
                $row_num++;//空一行
                $objPHPExcel->getActiveSheet()->mergeCells('A'.$row_num.':F'.$row_num);
            }
            $row_num++;
        }
        ...
        $outputFileName = "订单列表" . '_' . time() . '.xls';
//         $outputFileName = iconv('utf-8', "gb2312", $outputFileName);
        ...
        // var_dump("C:/Users/Administrator/Desktop/output".$outputFileName);
//         $xlsWriter->save("f://output//".$outputFileName,true);//写到永久文件
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if($_REQUEST['isJiFen'])
        {
        	if($_REQUEST['isJiFen']==1)
        	{
        		$where .= " AND o.extension_id !=0 ";
        	}else{
        		$where .= " AND o.extension_id =0 ";
        	}
        ...
        if ($filter['user_name'])
        {
            $sql = "SELECT COUNT(0)  as record_count,sum(o.integral) as total_integral, sum(goods_amount) as total_goods_amount, sum(o.shipping_fee) as total_shipping_fee FROM " . $GLOBALS['ecs']->table('order_info') . " AS o ,".
                   $GLOBALS['ecs']->table('users') . " AS u " . $where;
        }
        else
        {
            $sql = "SELECT  COUNT(0) as record_count,sum(o.integral) as total_integral, sum(goods_amount) as total_goods_amount, sum(o.shipping_fee) as total_shipping_fee FROM " . $GLOBALS['ecs']->table('order_info') . " AS o ". $where;
        }
   
        $row = $GLOBALS['db']->getRow($sql);
        $filter['record_count']   = $row['record_count'];
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        $filter['total_integral']     =  $row['total_integral'];//总的积分
        $filter['total_goods_amount']     =  $row['total_goods_amount'];//总的商品价格
        $filter['total_shipping_fee']     =  $row['total_shipping_fee'];//总的运费
        /* 查询 */
        if($filter['supp']){
        	$sql = "SELECT o.order_id,o.integral, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
