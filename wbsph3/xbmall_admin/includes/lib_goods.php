...
//        $filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
       ...
       if ($real_goods > -1)
        ...
        if(intval($_REQUEST['supp'])>0){
        	$sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,goods_store_number,exclusive, integral, " .
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number , goods_attr_price
