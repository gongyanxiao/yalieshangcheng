...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = " SELECT IFNULL(SUM(pay_points), 0) AS user_money, IFNULL(SUM(frozen_money), 0) AS frozen_money,IFNULL(SUM(history_zsq), 0) AS history_zsq,IFNULL(SUM(zsq), 0) AS zsq,IFNULL(SUM(tx_points), 0) AS tx_points FROM " .
    ...
    $account['tx_points']   = ($money_list['tx_points']);   //用户冻结金额
    
    $account['history_zsq']     = ($money_list['history_zsq']);   //用户可用余额
    $account['zsq']   = ($money_list['zsq']);   //用户冻结金额
    ...
    $sql="SELECT IFNULL(SUM(zsq),0) as zsq,IFNULL(SUM(history_zsq),0) as history_zsq,IFNULL(SUM(history_zsq-zsq),0) as zsqed FROM".$GLOBALS['ecs']->table('users')."";
    $zsqInfo=$GLOBALS['db']->getRow($sql);
    
    $account['history_zsqs']        = $zsqInfo["history_zsq"];
    $account['zsqs']        = $zsqInfo["zsq"];
    $account['zsqed']        = $zsqInfo["zsqed"];
