...
{
    /* 权限判断 */
    admin_priv('exchange_goods_list');
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    {
        $where = " AND g.goods_name LIKE '%" . mysql_like_quote($_REQUEST['keyword']) . "%'";
    }
    ...
    $sql = 'SELECT eg.* , g.goods_name '.
        ...
        'WHERE 1 ' .$where ." order by eg.goods_id desc  ";
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('C1', '使用积分值');
    $objPHPExcel->getActiveSheet()->setCellValue('D1', '是否可兑换');
    $objPHPExcel->getActiveSheet()->setCellValue('E1', '是否热销');
    ...
    foreach($res as $value)
    {
        ...
        // 设置价格为数字格式
        $objPHPExcel->getActiveSheet()->getStyle('D' . $row_num)->getAlignment()
        ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        // 居中
        $objPHPExcel->getActiveSheet()->getStyle('E' . $row_num)->getAlignment()
        ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C' . $row_num, $value['exchange_integral']);
        $objPHPExcel->getActiveSheet()->setCellValue('D' . $row_num, $value['is_exchange'] ==1 ? '√' : '×');
        $objPHPExcel->getActiveSheet()->setCellValue('E' . $row_num, $value['is_hot'] ==1 ? '√' : '×');
        $row_num++;
    }
