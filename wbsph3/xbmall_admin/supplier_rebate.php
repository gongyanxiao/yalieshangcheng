...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('offlines', array(array("code" => 1, "name" => "线上订单"), array("code" => 2, "name" => "线下报单")));
...
elseif ($_REQUEST['act'] == 'export_supps') {
    ...
    $export = "<table border='1'><tr><td colspan='2'>运营中心名称</td><td colspan='2'>商家名称</td><td colspan='2'>报单人</td><td colspan='2'>订单收入总额（元）</td><td colspan='2'>佣金抽成总额（元）</td><td colspan='2'>商家实际收入总额（元）</td></tr>";
    ...
    foreach ($result['result'] as $key => $val) {
        ...
        $export .= "<tr><td colspan='2'>" . $region_name . "</td><td colspan='2'>" . $val["supplier_name"] . "</td><td colspan='2'>" . $val["mobile_phone"].$real_name . "</td><td colspan='2'>" . $val['all_money'] . "</td><td colspan='2'>-" . $val['rebate_money'] . "</td><td colspan='2'>" . $val['result_money'] . "</td></tr>";
    }
    $export .= "</table>";
    if (EC_CHARSET != 'gbk') {
...
elseif ($_REQUEST['act'] == 'search_supp_query') {
    ...
    make_json_result($smarty->fetch('supplier_rebate_info.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
//导出
...
function supplier_rebate_list() {
    $result = get_filter();
    if ($result === false) {
        if (isset($_REQUEST['is_offline']) && !empty($_REQUEST['is_offline'])) {
            $filter['is_offline'] = $_REQUEST['is_offline'];
        }
        if (isset($_REQUEST['is_suppliers']) && !empty($_REQUEST['is_suppliers'])) {
            $filter['is_suppliers'] = $_REQUEST['is_suppliers'];
        }
        $filter['suppid'] = (isset($_REQUEST['suppid']) && intval($_REQUEST['suppid']) > 0) ? intval($_REQUEST['suppid']) : 0;
        $filter['sort_by'] = empty($_REQUEST['sort_by']) ? ' s.supplier_id' : trim($_REQUEST['sort_by']);
        ...
        if (!empty($filter['is_offline'])) {
            if ($filter['is_offline'] * 1 == 2) {
                $filter['is_offline'] = 1;
            } else {
                $filter['is_offline'] = 0;
            }
            $where .= " and sr.is_offline =" . $filter['is_offline'] . " ";
        }

        if (!empty($filter['is_suppliers'])) {
            if ($filter['is_suppliers'] * 1 == 1) {
                $where .= " and s.supplier_name <>'' ";
            } else {
                $where .= " and s.supplier_name is null ";
            }
        }
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(sr.supplier_id) FROM " . $GLOBALS['ecs']->table('supplier_rebate_log') . " AS sr  left join " . $GLOBALS['ecs']->table("supplier") . " AS s on sr.real_supplier_id = s.supplier_id 
                    LEFT JOIN  " . $GLOBALS['ecs']->table("region") . " AS a ON sr.supplier_parent_id = a.user_id 
LEFT JOIN " . $GLOBALS['ecs']->table("users") . " AS b ON sr.supplier_id = b.user_id " .
                $where . " group by sr.supplier_id ";
        $supp_type = $GLOBALS['db']->getAll($sql);
        $filter['record_count'] = count($supp_type);
        $filter['page_count'] = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        /* 查询 */
        $sql = "SELECT sum(sr.all_money) as all_money, sum(sr.rebate_money) as rebate_money, sum(sr.result_money) as result_money, b.mobile_phone,
	b.real_name,s.supplier_id, s.supplier_name, s.supplier_rebate,b.user_name,b.user_id,
	CONCAT(
		ifnull(
			a.region_name,
			(
				...
				FROM
					ecs_region t
				WHERE
					t.user_id = sr.supplier_id
			)
		),
		'运营中心'
	) AS region_name " .
                "FROM " . $GLOBALS['ecs']->table("supplier_rebate_log") . " AS  sr left join " . $GLOBALS['ecs']->table("supplier") . " AS s on sr.real_supplier_id = s.supplier_id 
                    LEFT JOIN  " . $GLOBALS['ecs']->table("region") . " AS a ON sr.supplier_parent_id = a.user_id AND sr.supplier_parent_id > 0 
LEFT JOIN " . $GLOBALS['ecs']->table("users") . " AS b ON sr.supplier_id = b.user_id
                $where
...
function supplier_rebate_info_list() {
    ...
    if ($result === false) {
        ...
        $filter['supplier_id'] = intval($_REQUEST['supplier_id']);
        if (isset($_REQUEST['is_offline']) && !empty($_REQUEST['is_offline'])) {
            $filter['is_offline'] = $_REQUEST['is_offline'];
        }
        ...
        if (!empty($filter['supplier_id'])) {
            $where.=" and sr.real_supplier_id=" . $filter['supplier_id'];
        }
        if ($filter['start_time']) {
            $where .= " and sr.add_time >= '" . $filter['start_time'] . "' ";
        }
...
function main_rebate_list() {
	...
	sum(
		...
		ELSE
			0
		END
	) AS result_money from " . $ecs->table('supplier_rebate_log');
...
function get_all_supplier() {
    global $db, $ecs;
    //$sql = "select s.user_id as supplier_id, s.supplier_name from " . $ecs->table('supplier_rebate_log') . " as srl left join " . $ecs->table('supplier') . " as s on srl.supplier_id=s.supplier_id group by srl.supplier_id";
    $sql = "SELECT srl.supplier_id AS supplier_id,IFNULL(s.supplier_name,CONCAT (a.region_name,'运营中心')) AS supplier_name  FROM " . $GLOBALS['ecs']->table('supplier_rebate_log') . " AS srl  LEFT JOIN " . $GLOBALS['ecs']->table("supplier") . " AS s ON srl.supplier_id = s.supplier_id LEFT JOIN " . $GLOBALS['ecs']->table("region") . " AS a ON srl.supplier_id = a.user_id GROUP BY	srl.supplier_id";
