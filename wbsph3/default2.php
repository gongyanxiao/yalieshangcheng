...
else
{
    ...
    {
        /* 代码修改_START   By       */
		include_once('includes/lib_splitword.php');
		$Recordkw = str_replace(array("\'"), array(''), trim($_REQUEST['keywords']));
		$encode = 'utf-8';
        $splitWord = new SplitWord($encode, $encode);
        $splitWord->SetSource($Recordkw, $encode, $encode);
        $splitWord->SetResultType(1);
        $splitWord->StartAnalysis(TRUE);
        $splitResult = $splitWord->GetFinallyResult(' '); 
		//echo  $word;
        $splitResult = preg_replace("/[ ]{1,}/", " ", trim($splitResult));
        $keyWordArr = explode(' ', $splitResult);
        ...
        {
            if ($key > 0 && $key < count($keyWordArr) && count($keyWordArr) > 1)
            {
                $keywords .= " OR ";
            }
    ...
    /*20161225添加商家信息开始*/
    $supplier_ids=" AND g.supplier_id!=0";
    /*20161225添加商家信息结束*/
   ...
   /* fulltext_search_add_END_ */
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        ...
        "AND (( 1 "  . $supplier_ids.$keywords . $brand . $min_price . $max_price . $intro . $outstock ." ) ".$tag_where.  area_sql_limit_cookie()." )";
    $count = $db->getOne($sql);
	/* 代码添加_START   By    */	
...
//	$ts_string_array = array("*","/");
    ...
    if($_CFG['fulltext_search'] == '1'){
        ...
        if(empty($ids)) $ids = '-1';
        $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in $categories  $supplier_ids".
        "AND (( 1 "  ."and g.goods_id in ({$ids})"." ) ".$tag_where.area_sql_limit_cookie()." )";
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['goods_name'] =  $arr[$row['goods_id']]['goods_name'];
		foreach($keyWordArr as $key =>$temp_qq)
		{
				//$replacet[$key]=  '<strong style="color:#cc0000;">'. $temp_qq .'</strong>';;
				
				if(in_array($temp_qq,$ts_string_array)){
					$arr[$row['goods_id']]['goods_name'] = preg_replace('/(?!<[^>]*)\\'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name']);
				}else{
					$arr[$row['goods_id']]['goods_name'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name']);
				}
	...
	*/
	...
	$smarty->assign('isSupplier','1');//商家
	//echo '<pre>';
	//print_r($cat_two_arr);
	//echo '</pre>';
    $smarty->display('default2.dwt');
}
