...
function create_shop_settiongs() {
    ...
    $sql = "SELECT count(id) FROM " . $ecs->table('supplier_shop_config') . " WHERE supplier_id=" . $_SESSION['supplier_id'] . " and `code`='shop_info'";
    $num = $db->getOne($sql);
    if ($num > 0) {
        return;
    } else {
        $insql = "INSERT INTO " . $ecs->table('supplier_shop_config') . " (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `supplier_id`) VALUES
				...
				(8, 0, 'sms', 'hidden', '', '', '', 1, " . $_SESSION['supplier_id'] . "),
...
function checkje($amount) {
    if (isset($amount) && !empty($amount)) {
        ...
        if ($amount > 0) {
            ...
            $zuodan_set_max = $zuodan_set_max * 1;
