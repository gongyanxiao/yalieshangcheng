...
//在89行  添加 
require(ROOT_PATH . 'includes/lib_uuecs_admin.php'); ////Powered By UUECS QQ364642382 HTTP://Www.UUECS.Com
/* 对用户传入的变量进行转义操作。*/
...
/* 验证管理员身份 */
if ((!isset($_SESSION['member_user_id']) || intval($_SESSION['member_user_id']) <= 0) &&
    $_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    /* session 不存在，检查cookie */
    if (!empty($_COOKIE['ECSCP']['supplier_id']) && !empty($_COOKIE['ECSCP']['member_user_id']) && !empty($_COOKIE['ECSCP']['supplier_pass']))
    {
        ...
        //$row = $db->GetRow($sql);
        $sql = 'SELECT user_id,uid, user_name, password, action_list, last_login, supplier_id ' .
                ' FROM ' .$ecs->table('supplier_admin_user') .
                " WHERE user_id = '" . intval($_COOKIE['ECSCP']['member_user_id']) . "' AND supplier_id=".intval($_COOKIE['ECSCP']['supplier_id']);
