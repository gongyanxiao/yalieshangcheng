...
if ($_REQUEST['act'] == 'list_edit') {
    ...
    $group_list = get_settings(array('1'), array(38, 39, 40, 41, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 117, 118, 119, 120, 121, 122, 123, 124, 125, 126, 127, 128, 129, 130));
...
//    if ($_CFG['shop_country'] > 0) {
//        if ($_CFG['shop_province']) {
//            $smarty->assign('cities', get_cities($_CFG['shop_province']));
//        }
//    }
...
function get_settings($groups = null, $excludes = null) {
    ...
    $sql = "SELECT * FROM " . $ecs->table('supplier_shop_config') .
            " WHERE supplier_id=" . $_SESSION['supplier_id'] . " AND type<>'hidden' " . $excludes_groups . " ORDER BY parent_id, sort_order, id";
...
//    $sql = "select `cat_id`,`cat_name` from " . $GLOBALS['ecs']->table('area') . " where `is_show`=1 and `parent_id`=1 order by `shortname` asc";
//    $provices = $GLOBALS['db']->getALL($sql);
//    $datas = array();
//    foreach ($provices as $key => $pro) {
//
//        $sql = "select `cat_id`,`cat_name` from " . $GLOBALS['ecs']->table('area') . " where `is_show`=1 and `parent_id`='" . $pro["cat_id"] . "' order by `shortname` desc";
 ...
 * @return type
 */
//function get_cities($cat_id) {
//
//    $sql = "select `cat_id`,`cat_name` from " . $GLOBALS['ecs']->table('area') . " where `is_show`=1 and `parent_id`='" . $cat_id . "' order by `shortname` desc";
//    $citys = $GLOBALS['db']->getALL($sql);
//    return $citys;
//}
?>