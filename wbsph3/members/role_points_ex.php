...
if ($_REQUEST['act'] == "add") {
    ...
    if (empty($profileRow["real_name"]) || empty($profileRow["card"]) || empty($profileRow["bank"]) || empty($profileRow['bank_kh']) || empty($profileRow['bank_num'])) {
        $links = array(
            array('href' => 'role_points_ex.php?act=info', 'text' => "个人信息")
        );
        sys_msg("请将个人信息补充完整，才能继续进行积分兑换操作", 0, $links);
    } else {
        $sql = "SELECT add_time FROM " . $GLOBALS['ecs']->table("user_account") . " where user_id='" . $_SESSION["member_uid"] . "' and process_type=1 and type=0 order by add_time DESC limit 1 ";
        ...
        assign_query_info();
        $smarty->display('role_points_exchange.htm');
    }
 ...
 */
    ...
    if ($num > 0) {
        $links = array(
            array('href' => 'role_points_ex.php?act=list', 'text' => "积分兑换")
        );
    ...
    $smarty->assign('full_page', 1);
    assign_query_info();
    $smarty->assign('ur_here', "积分兑换记录");
    $smarty->assign('action_link', array('text' => "积分兑换", 'href' => 'role_points_ex.php?act=add'));
    ...
    $smarty->display('role_points_exchanges.htm');
    ...
    /* 变量初始化 */
    $surplus = array(
        'user_id' => $_SESSION["member_uid"], 'rec_id' => !empty($_POST['rec_id']) ? intval($_POST['rec_id']) : 0, 'process_type' => isset($_POST['surplus_type']) ? intval($_POST['surplus_type']) : 0, 'payment_id' => isset($_POST['payment_id']) ? intval($_POST['payment_id']) : 0, 'user_note' => isset($_POST['user_note']) ? trim($_POST['user_note']) : '', 'amount' => $amount
    );
    /* 判断是否有足够的余额的进行退款的操作 */
    $userDefault = get_user_default($_SESSION["member_uid"]);
    $sur_amount = $userDefault["hk_points"] * 1;
    //0 1 2 3 4 5 6 
//    if (date("w") * 1 != 5) {//上一个提现日期间隔7天
//        $content = "提现日期每周五，此操作将不可进行！";
...
//     }
    ...
    if ($surplus['rec_id'] > 0) {
        ...
        $res1 = log_account_change($_SESSION["member_uid"], 0, 0, 0, 0, "商家PC端发起积分兑换操作", 1, 0, 0, 0, 0, 0, 0, 0, $amount, $amount * (-1), 0, 0);
        if ($res1) {
            $content = "您的积分兑换申请已经提交，请等待审核。";
    ...
    include_once (ROOT_PATH . 'includes/lib_order.php');
    if ($surplus_id == 0) {
        ecs_header("Location: role_points_ex.php?act=list\n");
        exit();
    }
    $GLOBALS['db']->startTrans();
    ...
    if ($result) {
        ...
        if ($account_order['process_type'] * 1 == 1) {
            $ress = log_account_change($_SESSION['member_uid'], 0, 0, 0, 0, "商家PC端手动取消积分兑换，货款积分增加", 1, 0, 0, 0, 0, 0, 0, 0, abs($amount), $amount, 0, 0);
        } else {
            $ress = true;
        }
        if ($ress) {
...
function get_exchange_account_log($user_id) {
    ...
    $where = " WHERE user_id = '$user_id' AND process_type ='1'  and type=0 ";
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (($rows['is_paid'] == 0) && ($rows['process_type'] == 1)) {
            $rows['handle'] = '<a href="role_points_ex.php?act=cancel&id=' . $rows['id'] . '" onclick="if(confirm(\'确定删除当前积分兑换记录?\')){return true;}else{return false;}">删除</a>';
            $rows['pay_id'] = $pid;
        }
        $arr[] = $rows;
    }
