...
function action_login() {
    ...
    if (!empty($_SESSION['temp_user_name'])) {
    	
        $sql = "SELECT	s.user_id,s.supplier_id,s.role,s.city_id,s.large_area_id,a.supplier_name,b.`name` as lname,c.region_name FROM
...
WHERE s.user_name='" . $_SESSION['temp_user_name'] . "'";
        $suppliers = $GLOBALS['db']->getAll($sql);
        foreach ($suppliers as $suppKey => $suppValue) {
            switch ($suppValue["role"]) {//周:商家后台角色
                case 1:
                    $suppliers[$suppKey]['real_name'] = "(大区经理)" . $suppValue['lname'];
                    break;
                case 2:
                    $suppliers[$suppKey]['real_name'] = "(城市经理)" . $suppValue['region_name'];
                    break;
                case 3:
                    $suppliers[$suppKey]['real_name'] = "(线上商家)" . $suppValue['supplier_name'];
                    break;
                case 5:
                	$suppliers[$suppKey]['real_name'] = "(大家庭代理)";
                	break;
                default :
                    $suppliers[$suppKey]['real_name'] = "(联盟商家)" . $suppValue['supplier_name'];
                    break;
            }
        }

        $smarty->assign("suppliers", $suppliers);
        $smarty->assign("temp_user_password", $_SESSION['temp_user_password']);
        $smarty->assign("temp_user_name", $_SESSION['temp_user_name']);
    }
...
//        }
    ...
    if (empty($supplier_id)) {
    	$sql = "SELECT count(*) FROM  " . $GLOBALS['ecs']->table('supplier_admin_user') . " s  WHERE s.user_name ='" . $user_name . "'  ";
    	$users = $GLOBALS['db']->getOne($sql);
    	if (($users) * 1 > 1) {
    		$_SESSION['temp_user_name'] = $user_name;
    		$_SESSION['temp_user_password'] = $_POST['password'];
    		//            $link[] = array(
    		//                'text' => "返回上一页", 'href' => "privilege.php?act=login&temp_user_name=" . $user_name
    		//            );
    		//            sys_msg("当前账号关联多个账号，请选择账号信息", 1, $link);
    ...
    if (empty($supplier_id)) {
        ...
        if (!empty($ec_salt)) {
            /* 检查密码是否正确 */
            $sql = "SELECT user_id, uid,user_name, password, last_login, action_list, last_login,supplier_id,ec_salt,role,large_area_id,city_id" . " FROM " . $ecs->table('supplier_admin_user') . " WHERE user_name = '" . $user_name . "' AND password = '" . md5(md5($_POST['password']) . $ec_salt) . "' AND checked=1";
        } else {
            /* 检查密码是否正确 */
            $sql = "SELECT user_id, uid,user_name, password, last_login, action_list, last_login,supplier_id,ec_salt,role,large_area_id,city_id" . " FROM " . $ecs->table('supplier_admin_user') . " WHERE user_name = '" . $user_name . "' AND password = '" . md5($_POST['password']) . "'  AND checked=1";
        }
    } else {
        $sql = "SELECT `ec_salt` FROM " . $ecs->table('supplier_admin_user') . " WHERE user_name = '" . $user_name . "' AND user_id='" . $supplier_id . "'";
        $ec_salt = $db->getOne($sql);
        if (!empty($ec_salt)) {
            /* 检查密码是否正确 */
            $sql = "SELECT user_id,uid, user_name, password, last_login, action_list, last_login,supplier_id,ec_salt,role,large_area_id,city_id" . " FROM " . $ecs->table('supplier_admin_user') . " WHERE user_name = '" . $user_name . "' AND password = '" . md5(md5($_POST['password']) . $ec_salt) . "' AND checked=1 AND user_id='" . $supplier_id . "'";
        } else {
            /* 检查密码是否正确 */
            $sql = "SELECT user_id, uid,user_name, password, last_login, action_list, last_login,supplier_id,ec_salt,role,large_area_id,city_id" . " FROM " . $ecs->table('supplier_admin_user') . " WHERE user_name = '" . $user_name . "' AND password = '" . md5($_POST['password']) . "'  AND checked=1 AND user_id='" . $supplier_id . "'";
        }
    }
    ...
    if ($row) {
        ...
        //查询是否加入小黑屋
        $sql="SELECT * FROM ".$GLOBALS['ecs']->table("user_blacklist")."   WHERE is_remove > 0 and user_id='".$row['uid']."'";
        $user_infos =$GLOBALS['db']->getRow($sql);
        if ($user_infos)
        {
            sys_msg('该账号已被加入小黑屋:'.$user_infos['limit_desc'].'(具体原因请联系管理员)');
            exit;
        }
        ...
        unset($_SESSION['temp_user_password']);
        $_SESSION['supplier_id'] = $row['supplier_id']; // 店铺的id
        $_SESSION['member_user_id'] = $row['user_id']; // 管理员id
        $_SESSION['member_uid'] = $row['uid']; // 会员id
        $_SESSION['supplier_name'] = $row['user_name']; // 管理员名称
      
        $sql="SELECT `action` FROM ".$GLOBALS['ecs']->table("users_role")."   WHERE id='".$row['role']."'";
        $action_list=$GLOBALS['db']->getOne($sql);
        //$_SESSION['supplier_action_list'] = $row['action_list']; // 管理员权限
        $_SESSION['supplier_action_list'] = $action_list; // 管理员权限
        $_SESSION['supplier_last_check'] = $row['last_login']; // 用于保存最后一次检查订单的时间
        $_SESSION['member_role'] = $row['role']; // 管理员id
        $_SESSION['member_large_area_id'] = $row['large_area_id']; // 管理员id
        $_SESSION['member_city_id'] = $row['city_id']; // 管理员id
        $_SESSION['member_city_level'] = $row['city_level']; // 管理员id
        ...
        if (isset($_POST['remember'])) {
            ...
            setcookie('ECSCP[member_user_id]', $row['user_id'], $time);
