...
$cache_id = sprintf('%X', crc32($cat_id . '-' . $display . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' .
                $_CFG['lang'] . '-' . $brand . '-' . $price_max . '-' . $price_min . '-' . $filter_attr_str . '-' . $filter . '-' . $is_stock . "-" . $_COOKIE['area_id']));
        ...
        if (!empty($filter_attr)) {
            ...
            foreach ($filter_attr AS $k => $v) {                      // 查出符合所有筛选属性条件的商品id */
                if (strstr($v, '_') && $v != 0 && isset($cat_filter_attr[$k])) {
                    ...
                    $sql = $ext_sql . "b.attr_value = a.attr_value AND b.attr_id = " . $cat_filter_attr[$k] . " AND a.goods_attr_id = " . intval($v);
                    $ext_group_goods = $db->getColCached($sql);
                    $ext .= ' AND ' . db_create_in($ext_group_goods, 'g.goods_id');
                }
            }
...
//
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style, g.click_count, g.goods_number, g.market_price, " .
            ...
            " WHERE $where $ext " .
            " AND ( $area_cat_all_sql $where2 ) " . //Powered By UUECS QQ364642382 HTTP://Www.UUECS.Com
            " ORDER BY $area_order $sort $order";
    if ($sort == 'salenum') {
...
//    }
    ...
    $filter = (isset($_REQUEST['filter'])) ? intval($_REQUEST['filter']) : 0;
    $where = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND " . "  ( $area_cat_all_sql $where2 ) " . " AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
