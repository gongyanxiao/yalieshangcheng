...
/* 代码增加_end   By www.ecshop68.com */
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
		...
		foreach ($properties['spe'] AS $skey_ecshop68=>$sval_ecshop68)
		{
			...
			{				
				...
				{ 
					...
					$sskey = ($sskey != '-1') ? $sskey : $sskey_ecshop68;
				}
				else
				{
			...
			if ($sskey!='-1')
			{
				$properties['spe'][$skey_ecshop68]['values'][$sskey]['selected_key_ecshop68'] = "1";
			}
		}
		...
		if ($rank_num['rank_total'] > 0) { $nums = 0 ;}else{ $nums = 100 ;} 
		$rank_num['rank_pa'] = ($rank_num['rank_a'] > 0) ? round(($rank_num['rank_a'] / $rank_num['rank_total']) * 100,1) : $nums;
		$rank_num['rank_pb'] = ($rank_num['rank_b'] > 0) ? round(($rank_num['rank_b'] / $rank_num['rank_total']) * 100,1) : $nums;
		$rank_num['rank_pc'] = ($rank_num['rank_c'] > 0) ? round(($rank_num['rank_c'] / $rank_num['rank_total']) * 100,1) : $nums;
