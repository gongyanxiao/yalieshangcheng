...
function insert_user_account($surplus, $amount) {
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('user_account') .
            ...
            " VALUES ('$surplus[user_id]', '$admin_user', '$amount', '" . gmtime() . "', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0)";
    $GLOBALS['db']->query($sql);
 ...
 * @return  array
 */
function get_account_log($user_id, $num, $start, $ptype = "") {
    $account_log = array();
    if ($ptype === "") {
        $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('user_account') .
                " WHERE user_id = '$user_id'" .
...
function get_user_default($user_id) {
    ...
    $info['tx_points'] = $row['tx_points']; //成功提现积分
